...
async function discover(request,env){
  ...
  for(const s of sources){
    ...
    for(const raw of allLinks.slice(0,c.guardrails?.max_candidates_per_source||50)){
      ...
      const baseScore=x.type==='agent_registry'?72:x.type==='api_directory'?68:58;
      const family=families.get(x.type),familyBoost=Math.max(0,Number(family?.boost||0));
      const score=Number(Math.min(100,baseScore+familyBoost).toFixed(2));if(familyBoost>0)familyBoosted++;
      await env.DB.prepare(`INSERT OR IGNORE INTO distribution_opportunities(surface_slug,surface_name,surface_type,audience_fit,authority,traffic_potential,backlink_value,acceptance_probability,automation_potential,effort_cost,distribution_score,status,action_url,human_required,last_checked_at,next_action,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),?,datetime('now'),datetime('now'))`).bind(x.slug,x.name,x.type,x.type==='agent_registry'?88:65,x.type==='agent_registry'?75:55,x.type==='agent_registry'?78:60,60,50,x.type==='agent_registry'||x.type==='api_directory'?70:45,45,score,x.status,x.url,x.human_required,`${x.next_action} Family-learning boost ${familyBoost.toFixed(2)} from ${Number(family?.evidence||0)} proven surface(s); unseen families retain their full baseline score.`).run();known.add(x.slug);inserted++;
    }
