...
import {normalizeAffiliateState, MONETIZED_STATES} from './affiliate-operations.js';

const MAX_TOOLS_PER_CYCLE=4;
const MAX_CANDIDATES_PER_TOOL=5;
...
function publicHttpUrl(value){
  try{
    ...
    if(h==='localhost'||h.endsWith('.localhost')||h.endsWith('.local')||h==='0.0.0.0'||h==='127.0.0.1'||h==='::1'||/^10\./.test(h)||/^192\.168\./.test(h)||/^169\.254\./.test(h)||/^172\.(1[6-9]|2\d|3[01])\./.test(h)) return null;
    return u;
  }catch{return null;}
}
...
function linksFromHtml(html,base){const out=[];const re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;while((m=re.exec(String(html||'')))&&out.length<80){try{const u=new URL(m[1],base);if(sameSite(u.hostname,new URL(base).hostname)&&PROGRAM_WORDS.test(`${m[1]} ${stripHtml(m[2])}`))out.push(u.href)}catch{}}return [...new Set(out)];}
async function boundedFetch(url){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),FETCH_TIMEOUT_MS);try{const r=await fetch(url,{method:'GET',redirect:'follow',headers:{'User-Agent':'ToolScout-Affiliate-Coverage/1.0 (+https://trytoolscout.org/)'},signal:ctl.signal});if(!r.ok)return null;const type=r.headers.get('content-type')||'';if(!type.includes('text/html')&&!type.includes('text/plain'))return null;const text=(await r.text()).slice(0,600000);const final=publicHttpUrl(r.url);if(!final)return null;return {url:final.href,html:text,text:stripHtml(text)};}catch{return null}finally{clearTimeout(timer)}}
...
async function discoverOfficialProgram(tool){
  ...
  const directLinks=linksFromHtml(first.html,first.url);
  const candidates=[...directLinks];
  for(const p of ['/affiliate','/affiliates','/affiliate-program','/partners','/referral-program']){try{candidates.push(new URL(p,home.origin).href)}catch{}}
  for(const candidate of [...new Set(candidates)].slice(0,MAX_CANDIDATES_PER_TOOL)){
    ...
    if(!PROGRAM_WORDS.test(evidence))continue;
...
async function persistDiscovery(env,slug,result){
  ...
  const existing=await env.DB.prepare('SELECT status FROM affiliate_workflow WHERE tool_slug=?').bind(slug).first();
  const current=normalizeAffiliateState(existing?.status);
  if(['research_required','program_exists','ready_to_apply'].includes(current)){
...
export async function runAffiliateCoverageCycle(env){
  const [tools,workflow,clicks]=await Promise.all([
    loadTools(env),
    safeAll(env,'SELECT * FROM affiliate_workflow'),
    safeAll(env,"SELECT c.tool_slug,c.affiliate_active_at_click,c.source,COALESCE(s.classification,'unknown/legacy') classification,COUNT(*) clicks FROM click_events c LEFT JOIN sessions s ON s.session_id=c.session_id WHERE c.created_at>=datetime('now','-30 days') GROUP BY c.tool_slug,c.affiliate_active_at_click,c.source,classification")
  ]);
  const states=new Map((workflow.results||[]).map(r=>[r.tool_slug,r]));
  const records=tools.map(t=>{const s=states.get(t.slug)||{};return {...t,status:normalizeAffiliateState(s.status),network:s.network||null,blocker:s.blocker||null,submitted_at:s.submitted_at||null};});
  const snapshot=coverageEngineSnapshot(records,clicks.results||[]);
  ...
  let researched=0,found=0,human=0;
  for(const queued of snapshot.recoverable_queue.slice(0,MAX_TOOLS_PER_CYCLE)){
