...
import {publicCatalogInventory} from './catalog-autonomy-worker.js';
...
metric('Catalog tools',n(x.tools),'Canonical deduplicated catalog')+
metric('Affiliate coverage',n(x.affiliate_catalog_coverage_pct)+'%',n(x.affiliate_active_tools)+' active · '+n(x.affiliate_uncovered_tools)+' uncovered')+
...
function autonomousGrowthScript(){return `<script>(function(){const n=v=>Number(v||0).toLocaleString(),p=v=>Number(v||0).toFixed(0)+'%',esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])),when=v=>{try{return new Date(String(v||'').replace(' ','T')+'Z').toLocaleString()}catch{return String(v||'')}};function executionLog(items,total){items=Array.isArray(items)?items:[];if(!items.length)return '<details class="row" style="margin-top:10px"><summary class="rowName" style="cursor:pointer">External execution log · '+n(total)+'</summary><div class="rowMeta" style="margin-top:8px">No execution-level records available.</div></details>';return '<details class="row" style="margin-top:10px"><summary style="cursor:pointer"><span class="rowName">External execution log · '+n(total)+'</span><span class="rowMeta" style="margin-left:8px">Open to audit each action</span></summary><div style="margin-top:10px">'+items.map(function(i){var href=i.external_url||i.target_url||'';var action=href?'<a class="btn" style="margin-top:7px;display:inline-block" href="'+esc(href)+'" target="_blank" rel="noopener">Open evidence</a>':'';return '<div class="task" style="margin-top:8px"><div class="rowName">'+esc(i.label||i.type||i.id)+'</div><div class="rowMeta">'+esc(String(i.engine||'unknown'))+' · '+esc(String(i.type||'execution'))+' · '+esc(String(i.channel||'n/a'))+' · '+esc(String(i.status||'observed'))+' · '+esc(when(i.at))+'</div>'+(i.detail?'<div class="taskReason" style="margin-top:5px">'+esc(i.detail)+'</div>':'')+action+'</div>'}).join('')+'</div></details>'}function rndDetails(items,total){items=Array.isArray(items)?items:[];if(!items.length)return '<div class="row"><div><div class="rowName">Growth R&D</div><div class="rowMeta">No active bounded experiments.</div></div><div class="value">0</div></div>';return '<details class="row" open><summary style="cursor:pointer"><span class="rowName">Growth R&D · '+n(total)+'</span><span class="rowMeta" style="margin-left:8px">Evidence · hypothesis · action · owner · status · result</span></summary><div style="margin-top:10px">'+items.map(function(i){var actions=(Array.isArray(i.action_steps)?i.action_steps:[]).map(function(a){return esc(String(a).replaceAll('_',' '))}).join(' · ');var owners=esc(i.owner||'Growth Brain');var user=i.human_required?'Human action is required only through the Chairman Queue when a concrete verified gate is present.':'No action required from you.';return '<div class="task" style="margin-top:8px"><div class="rowName">'+esc(i.title||i.experiment_type||i.id)+'</div><div class="rowMeta" style="margin-top:5px"><b>Evidence:</b> '+esc(i.evidence_summary||'No evidence summary available.')+'</div><div class="rowMeta" style="margin-top:5px"><b>Hypothesis:</b> '+esc(i.hypothesis||'')+'</div><div class="rowMeta" style="margin-top:5px"><b>Action:</b> '+(actions||'No executable action bound')+'</div><div class="rowMeta" style="margin-top:5px"><b>Owner:</b> '+owners+'</div><div class="rowMeta" style="margin-top:5px"><b>Status:</b> '+esc(String(i.status||'unknown').replaceAll('_',' '))+' · source '+(i.source_bound?'bound':'missing')+' · execution contract '+(i.contract_bound?'bound':'missing')+'</div><div class="rowMeta" style="margin-top:5px"><b>Result:</b> '+esc(i.result||'No measured result yet.')+'</div><div class="note" style="margin-top:7px">'+esc(user)+'</div></div>'}).join('')+'</div></details>'}function frontierDetails(items,total){items=Array.isArray(items)?items:[];if(!items.length)return '<div class="row"><div><div class="rowName">Growth R&D frontier</div><div class="rowMeta">No net-new acquisition ideas are currently queued.</div></div><div class="value">0</div></div>';return '<details class="row" open><summary style="cursor:pointer"><span class="rowName">Growth R&D frontier · '+n(total)+'</span><span class="rowMeta" style="margin-left:8px">New acquisition ideas · safe builds auto-deploy without pre-approval</span></summary><div style="margin-top:10px">'+items.map(function(i){var steps=(Array.isArray(i.action_steps)?i.action_steps:[]).map(function(a){return esc(String(a).replaceAll('_',' '))}).join(' · '),dep=i.deployment||null,depLine=dep?'<div class="rowMeta" style="margin-top:5px"><b>Deployment:</b> '+esc(String(dep.status||'unknown').replaceAll('_',' '))+(dep.deployed_at?' · '+esc(when(dep.deployed_at)):'')+(dep.result_summary?' · '+esc(dep.result_summary):'')+'</div>':'';return '<div class="task" style="margin-top:8px"><div class="rowName">'+esc(i.title||i.id)+'</div><div class="rowMeta" style="margin-top:5px"><b>Mechanism:</b> '+esc(i.mechanism||'')+'</div><div class="rowMeta" style="margin-top:5px"><b>Why it may work:</b> '+esc(i.hypothesis||'')+'</div><div class="rowMeta" style="margin-top:5px"><b>Automation:</b> '+n(i.automation_score)+'/100 · semi-passive '+n(i.semi_passive_score)+'/100 · '+esc(String(i.implementation_mode||'').replaceAll('_',' '))+'</div><div class="rowMeta" style="margin-top:5px"><b>Next:</b> '+esc(i.next_step||'Research and bind a safe executor.')+'</div><div class="rowMeta" style="margin-top:5px"><b>Execution path:</b> '+(steps||'One-time capability build required')+'</div><div class="rowMeta" style="margin-top:5px"><b>Signal:</b> '+esc(String(i.expected_signal||'traffic impact').replaceAll('_',' '))+'</div>'+depLine+'<div class="note" style="margin-top:7px">'+esc(i.user_action||'No recurring owner action expected after setup.')+'</div></div>'}).join('')+'</div></details>'}
function deploymentDetails(items){items=Array.isArray(items)?items:[];if(!items.length)return '<div class="row"><div><div class="rowName">Growth R&D deployments</div><div class="rowMeta">Auto-deploy is active. No R&D build has been recorded in the deployment ledger yet.</div></div><div class="value">armed</div></div>';return '<details class="row" open><summary style="cursor:pointer"><span class="rowName">Growth R&D deployments · '+n(items.length)+'</span><span class="rowMeta" style="margin-left:8px">Deployed · measuring · scaled · retired</span></summary><div style="margin-top:10px">'+items.slice(0,12).map(function(i){return '<div class="task" style="margin-top:8px"><div class="rowName">'+esc(i.title||i.idea_key)+'</div><div class="rowMeta"><b>Status:</b> '+esc(String(i.status||'unknown').replaceAll('_',' '))+(i.deployed_at?' · deployed '+esc(when(i.deployed_at)):'')+(i.last_measured_at?' · measured '+esc(when(i.last_measured_at)):'')+'</div>'+(i.result_summary?'<div class="taskReason">'+esc(i.result_summary)+'</div>':'')+(i.verified_url?'<a class="btn" style="margin-top:7px;display:inline-block" href="'+esc(i.verified_url)+'" target="_blank" rel="noopener">Open deployment</a>':'')+'</div>'}).join('')+'</div></details>'}function renderAutonomousGrowth(d){const r=document.getElementById('autonomousGrowthBody');if(!r)return;const x=d?.growthOps?.autonomousGrowth;if(!x||x.status!=='observed'){r.innerHTML='<div class="empty">Autonomous growth evidence is unavailable.</div>';return}r.innerHTML='<div class="grid4">'+
...
async function growthOpsSnapshot(request,env,ctx,stats){
  const [affiliateLatest,affiliateWeekOld,affiliateStatuses,affiliateDiscovery,affiliatePacks,affiliateRoutes,distributionStatuses,distribution24,distribution7,deliveryStates,distEvents,affiliateHistory,gsc,gscReality,sitemap,contentIntel,organicGrowth,aeoGeo,machineReadability,catalogFreshness,catalogHealth,toolProfileHolds,catalogRuntimeState,catalogRuntimeCandidates,catalogInventory,catalogRuntimeGaps,catalogRecentAdmissions,latestAudienceEvent,latestContentPublish,distributionNetworkStates,distributionPlacements]=await Promise.all([
    ...
    publicCatalogInventory(env).catch(()=>null),
  ...
  const [growthCardCore,actionImpact,growthRndRows,growthRndSourceRows,growthRndExecutionRows,growthRndFrontierRows,growthRndFrontierAsset,growthRndDeploymentsAsset]=await Promise.all([
    safeFirst(env,`WITH
      growth AS (
        SELECT COUNT(*) active,
    ...
    safeAll(env,`SELECT idea_key,title,mechanism,hypothesis,automation_score,semi_passive_score,implementation_mode,status,expected_signal,next_step,action_json,evidence_json,last_evaluated_at,updated_at FROM growth_rnd_frontier WHERE status<>'implemented' ORDER BY automation_score DESC,semi_passive_score DESC,updated_at DESC LIMIT 20`),
    assetJson(request,env,'/data/growth-rnd-frontier.json',{ideas:[]}),
    assetJson(request,env,'/data/growth-rnd-deployments.json',{version:1,policy:'auto_deploy_safe_bounded',preApprovalRequired:false,reporting:'command_center_plus_post_deploy_email',items:[]})
  ]);
  ...
  const growthRndFrontierSource=(growthRndFrontierRows||[]).length?(growthRndFrontierRows||[]):((growthRndFrontierAsset?.ideas||[]).map(idea=>({
    idea_key:idea.ideaKey,title:idea.title,mechanism:idea.mechanism,hypothesis:idea.hypothesis,
    automation_score:idea.automationScore,semi_passive_score:idea.semiPassiveScore,
    implementation_mode:idea.implementationMode,status:'candidate',expected_signal:idea.expectedSignal,
    next_step:idea.nextStep,action_json:JSON.stringify(idea.steps||[]),evidence_json:JSON.stringify({source:'frontier_asset_fallback',priority:idea.priority||0,cost_class:idea.costClass||'free'})
  })));
  const growthRndDeploymentItems=(Array.isArray(growthRndDeploymentsAsset?.items)?growthRndDeploymentsAsset.items:[])
    .map(item=>({
      idea_key:item.ideaKey||item.idea_key||null,
      title:item.title||item.ideaKey||item.idea_key||'Growth R&D deployment',
      status:item.status||'unknown',
      commit_sha:item.commitSha||item.commit_sha||null,
      deployed_at:item.deployedAt||item.deployed_at||null,
      verified_url:item.verifiedUrl||item.verified_url||null,
      result_summary:item.resultSummary||item.result_summary||null,
      baseline:item.baseline||null,
      current_metrics:item.currentMetrics||item.current_metrics||null,
      last_measured_at:item.lastMeasuredAt||item.last_measured_at||null,
      email_reported_at:item.emailReportedAt||item.email_reported_at||null
    }))
    .sort((a,b)=>timeMs(b.deployed_at||b.last_measured_at)-timeMs(a.deployed_at||a.last_measured_at));
  const growthRndDeploymentMap=new Map(growthRndDeploymentItems.filter(x=>x.idea_key).map(x=>[String(x.idea_key),x]));
  const growthRndFrontierItems=growthRndFrontierSource.filter(row=>String(row.status||'candidate')!=='implemented').map(row=>{
    const steps=parseJson(row.action_json||'[]',[]);
    return {
      id:row.idea_key||null,
      title:row.title||row.idea_key||'Acquisition idea',
      mechanism:row.mechanism||'Net-new acquisition mechanism',
      hypothesis:row.hypothesis||'',
      automation_score:n(row.automation_score),
      semi_passive_score:n(row.semi_passive_score),
      implementation_mode:row.implementation_mode||'one_time_build',
      status:row.status||'candidate',
      expected_signal:row.expected_signal||null,
      next_step:row.next_step||null,
      action_steps:Array.isArray(steps)?steps:[],
      user_action:String(row.implementation_mode||'')==='external_account_once'?'A one-time external account connection may be required after the build is ready.':'No recurring owner action is expected after setup.',
      deployment:growthRndDeploymentMap.get(String(row.idea_key||''))||null,
      last_evaluated_at:row.last_evaluated_at||row.updated_at||null
    };
  }).sort((a,b)=>(b.automation_score+b.semi_passive_score)-(a.automation_score+a.semi_passive_score)).slice(0,12);
