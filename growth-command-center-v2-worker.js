...
rndDetails(x.rnd_items,x.rnd_experiments)+frontierDetails(x.rnd_frontier_items,x.rnd_frontier_count)+'<div class="row"><div><div class="rowName">Chairman Queue</div><div class="rowMeta">Human intervention should trend down as machine-resolvable routes are learned.</div></div><div class="value">'+n(x.chairman_queue)+'</div></div>'+
...
async function growthOpsSnapshot(request,env,ctx,stats){
  ...
  const [growthCardCore,actionImpact,growthRndRows,growthRndSourceRows,growthRndExecutionRows,growthRndFrontierRows,growthRndFrontierAsset]=await Promise.all([
    safeFirst(env,`WITH
      growth AS (
        SELECT COUNT(*) active,
