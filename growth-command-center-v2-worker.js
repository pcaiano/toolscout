...
function catalogGrowthScript(){return `<script>(function(){const n=v=>Number(v||0).toLocaleString(),esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));function draw(d){const root=document.getElementById('catalogGrowthBody'),meta=document.getElementById('catalogGrowthMeta'),x=d?.growthOps?.engines?.catalog;if(!root)return;if(!x){root.innerHTML='<div class="empty">Catalog growth evidence unavailable.</div>';return}if(meta)meta.textContent=x.report_age_days==null?'Shared growth brain':'Evidence '+n(x.report_age_days)+'d old';root.innerHTML='<div class="metricGrid">'+
...
async function growthOpsSnapshot(request,env,ctx,stats){
  const [affiliateLatest,affiliateWeekOld,affiliateStatuses,affiliateDiscovery,distributionStatuses,distribution24,distribution7,deliveryStates,distEvents,affiliateHistory,gsc,sitemap,contentIntel,organicGrowth,aeoGeo,machineReadability,latestAudienceEvent,latestContentPublish,distributionNetworkStates,distributionPlacements]=await Promise.all([
  ...
  const [growthState,contactRouteState,autonomyEvents,humanEvents,actionImpact]=await Promise.all([
    safeFirst(env,`SELECT COUNT(*) active,SUM(CASE WHEN subject_type='tool' THEN 1 ELSE 0 END) tools,SUM(CASE WHEN subject_type='surface' THEN 1 ELSE 0 END) surfaces,SUM(CASE WHEN subject_type='search' THEN 1 ELSE 0 END) search,MAX(last_evaluated_at) last_evaluated_at FROM growth_opportunity_state WHERE status='active'`),
