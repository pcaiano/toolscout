...
'<div class="row" style="margin-top:10px"><div><div class="rowName">Growth loop health</div><div class="rowMeta">'+esc(String(x.loop_status||'unknown').toUpperCase())+' · '+n(x.loop_orphan_routes)+' orphan routes · '+n(x.loop_stale_route_actions)+' stalled route actions · '+n(x.loop_stale_growth_actions)+' stale actions · '+n(x.loop_failed_core_runs_24h)+' failed core runs</div></div><div class="value">'+esc(String(x.loop_status||'unknown'))+'</div></div>'+
'<div class="row"><div><div class="rowName">Alternate distribution routes</div><div class="rowMeta">Public contact or amplification routes discovered across '+n(x.contact_route_surfaces)+' external surfaces. '+n(x.route_actions_in_progress)+' in progress · '+n(x.route_actions_verified)+' verified impact · '+n(x.route_actions_stalled)+' stalled. These are opportunities to pursue, not external proof.</div></div><div class="value">'+n(x.contact_routes)+'</div></div>'+
...
async function growthOpsSnapshot(request,env,ctx,stats){
  ...
  const [growthState,growthRnd,contactRouteState,routeActionState,autonomyEvents,humanEvents,actionImpact]=await Promise.all([
    ...
    safeFirst(env,`SELECT COUNT(*) routes,COUNT(DISTINCT surface_slug) surfaces FROM distribution_contact_routes WHERE status IN ('discovered','in_loop')`),
    safeFirst(env,`SELECT COUNT(*) total,
      ...
      SUM(CASE WHEN status='verified_impact' THEN 1 ELSE 0 END) verified,
  ...
  const growthLoopWarning=n(staleRouteActions?.n)>0||n(staleGrowthActions?.n)>0||n(routeActionState?.stalled)>0;
  ...
  const autonomousGrowth={status:'observed',window_days:30,active_opportunities:n(growthState?.active),tool_opportunities:n(growthState?.tools),surface_opportunities:n(growthState?.surfaces),affiliate_opportunities:n(growthState?.affiliate),catalog_opportunities:n(growthState?.catalog),news_opportunities:n(growthState?.news),search_opportunities:n(growthState?.search),rnd_experiments:n(growthRnd?.active),rnd_last_evaluated_at:growthRnd?.last_evaluated_at||null,last_evaluated_at:growthState?.last_evaluated_at||null,autonomous_actions_7d:autonomousActions,human_interventions_7d:humanInterventions,autonomy_rate_pct:autonomyDenominator?Number((autonomousActions/autonomyDenominator*100).toFixed(1)):0,chairman_queue:n(queue.total),contact_routes:n(contactRouteState?.routes),contact_route_surfaces:n(contactRouteState?.surfaces),route_actions_total:n(routeActionState?.total),route_actions_queued:n(routeActionState?.queued),route_actions_in_progress:n(routeActionState?.in_progress),route_actions_verified:n(routeActionState?.verified),route_actions_human:n(routeActionState?.human),route_actions_auth:n(routeActionState?.auth),route_actions_stalled:n(routeActionState?.stalled),route_actions_exhausted:n(routeActionState?.exhausted),loop_status:growthLoopStatus,loop_last_completed_at:growthLoopRun?.last_completed_at||null,loop_age_hours:growthLoopAgeHours,loop_orphan_routes:n(orphanRoutes?.n),loop_stale_route_actions:n(staleRouteActions?.n),loop_stale_growth_actions:n(staleGrowthActions?.n),loop_empty_opportunity_actions:n(emptyOpportunityActions?.n),loop_failed_core_runs_24h:n(failedCoreRuns?.n),verified_placements:n(distributionPlacements?.placements),verified_backlinks:n(distributionPlacements?.backlinks),prepared_growth_actions_30d:n(actionImpact?.preparedActions),attributed_growth_actions_30d:n(actionImpact?.attributedActions),attributed_human_sessions_30d:n(actionImpact?.browserConfirmedSessions),attributed_outbound_30d:n(actionImpact?.outboundClicks),attributed_monetized_outbound_30d:n(actionImpact?.monetizedOutbound),attribution_rule:actionImpact?.attribution||'Exact growth action marker plus browser-confirmed likely-human session. Missing evidence is never counted as impact.'};
