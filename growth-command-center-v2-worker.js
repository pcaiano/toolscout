...
function affiliateCoverageWidget(){return `<section class="widget" data-widget="affiliate-status" style="--w:12;--h:6"><div class="widgetHead"><div><div class="widgetKicker">Affiliate · status · clicks</div><div class="widgetTitle">Affiliate Coverage Status</div></div><div class="widgetMeta">Verified outbound · 30d</div></div><div class="widgetBody" id="affiliateCoverageStatusBody"><div class="empty">Refresh to load affiliate status.</div></div><div class="resizeHandle"></div></section>`}
...
function autonomousGrowthScript(){return `<script>(function(){const n=v=>Number(v||0).toLocaleString(),p=v=>Number(v||0).toFixed(0)+'%',esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])),when=v=>{try{return new Date(String(v||'').replace(' ','T')+'Z').toLocaleString()}catch{return String(v||'')}};function executionLog(items,total){items=Array.isArray(items)?items:[];if(!items.length)return '<details class="row" style="margin-top:10px"><summary class="rowName" style="cursor:pointer">External execution log · '+n(total)+'</summary><div class="rowMeta" style="margin-top:8px">No execution-level records available.</div></details>';return '<details class="row" style="margin-top:10px"><summary style="cursor:pointer"><span class="rowName">External execution log · '+n(total)+'</span><span class="rowMeta" style="margin-left:8px">Open to audit each action</span></summary><div style="margin-top:10px">'+items.map(function(i){var href=i.external_url||i.target_url||'';var action=href?'<a class="btn" style="margin-top:7px;display:inline-block" href="'+esc(href)+'" target="_blank" rel="noopener">Open evidence</a>':'';return '<div class="task" style="margin-top:8px"><div class="rowName">'+esc(i.label||i.type||i.id)+'</div><div class="rowMeta">'+esc(String(i.engine||'unknown'))+' · '+esc(String(i.type||'execution'))+' · '+esc(String(i.channel||'n/a'))+' · '+esc(String(i.status||'observed'))+' · '+esc(when(i.at))+'</div>'+(i.detail?'<div class="taskReason" style="margin-top:5px">'+esc(i.detail)+'</div>':'')+action+'</div>'}).join('')+'</div></details>'}function renderAutonomousGrowth(d){const r=document.getElementById('autonomousGrowthBody');if(!r)return;const x=d?.growthOps?.autonomousGrowth;if(!x||x.status!=='observed'){r.innerHTML='<div class="empty">Autonomous growth evidence is unavailable.</div>';return}r.innerHTML='<div class="grid4">'+
...
'<div class="metric"><small>External proof</small><b>'+n(x.verified_placements)+'</b><span>'+n(x.verified_backlinks)+' verified backlinks</span></div></div>'+
executionLog(x.external_execution_items,x.external_executions_7d)+
'<div class="row" style="margin-top:10px"><div><div class="rowName">Backlink acquisition</div><div class="rowMeta">'+n(x.verified_backlinks)+' verified backlinks · '+n(x.verified_referring_domains)+' / '+n(x.backlink_bootstrap_floor)+' bootstrap referring domains · quality only · no paid ranking links · no reciprocal requirement</div></div><div class="value">'+(x.backlink_acquisition_required?'active':'monitoring')+'</div></div>'+
...
'<div class="row"><div><div class="rowName">Growth R&D</div><div class="rowMeta">Bounded autonomous experiments auditing new growth and monetization routes.</div></div><div class="value">'+n(x.rnd_experiments)+'</div></div><div class="row"><div><div class="rowName">Chairman Queue</div><div class="rowMeta">Human intervention should trend down as machine-resolvable routes are learned.</div></div><div class="value">'+n(x.chairman_queue)+'</div></div>'+
