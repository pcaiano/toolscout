...
'<div class="metric"><small>Active opportunities</small><b>'+n(x.active_opportunities)+'</b><span>'+n(x.tool_opportunities)+' tool · '+n(x.surface_opportunities)+' surface</span></div>'+
...
async function growthOpsSnapshot(request,env,ctx,stats){
  ...
  const [growthState,contactRouteState,autonomyEvents,humanEvents,actionImpact]=await Promise.all([
    safeFirst(env,`SELECT COUNT(*) active,SUM(CASE WHEN subject_type='tool' THEN 1 ELSE 0 END) tools,SUM(CASE WHEN subject_type='surface' THEN 1 ELSE 0 END) surfaces,MAX(last_evaluated_at) last_evaluated_at FROM growth_opportunity_state WHERE status='active'`),
