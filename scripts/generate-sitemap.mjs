...
const urls=[BASE+'/',`${BASE}/guides.html`,...CORE_PAGES.filter(file=>fs.existsSync(path.join(ROOT,file))&&isIndexable(path.join(ROOT,file))).map(file=>`${BASE}/${file}`)];
...
console.log(JSON.stringify({urls:unique.length,intentAndComparisonPages:files.length,indexableBlogPages:blogFiles.length,blogIndexIncluded:urls.includes(`${BASE}/blog/`),softwareTrendsIndexIncluded:unique.includes(`${BASE}/software-trends-index.html`)}));
