...
const tools = fs.existsSync('data/tools.json') ? JSON.parse(fs.readFileSync('data/tools.json','utf8')) : [];
...
function hostEntity(value) {
  try {
    const host = new URL(value).hostname.toLowerCase().replace(/^www\./,'');
    const first = host.split('.')[0] || '';
    return compactEntity(first);
  } catch {
    return '';
  }
}
...
for (const tool of tools) {
  for (const value of [tool?.slug, tool?.name]) {
    const key = compactEntity(value);
    if (key) catalogEntityKeys.add(key);
  }
  const host = hostEntity(tool?.sourceUrl);
  if (host) catalogEntityKeys.add(host);
}
...
function candidateType(slug) {
  if (/^best-/.test(slug) || /(?:^|-)(?:vs|versus|compare|comparison|alternatives?)(?:-|$)/.test(slug)) return 'content-intent';
  return 'software-entity';
}

function alreadyCovered(slug) {
  if (current.has(slug) || redirected.has(slug)) return true;
  if (candidateType(slug) !== 'software-entity') return false;
  const key = compactEntity(slug);
  return Boolean(key && catalogEntityKeys.has(key));
}
...
async function fetchText(url) {
  ...
  try {
    const res = await fetch(url, {
      headers: { 'user-agent': 'ToolScoutOrganicGrowthBot/1.2 (+https://trytoolscout.org)' },
      redirect: 'follow',
...
for (const source of sources) {
  for (const url of source.urls) {
    ...
    if (!slug) continue;
    if (alreadyCovered(slug)) { suppressedCovered += 1; continue; }
    const row = candidateMap.get(slug) || { slug, candidateType: candidateType(slug), entityKey: entityKey(slug), mentions: 0, sources: [], exampleUrls: [] };
...
fs.writeFileSync('reports/competitive-gap-signals.json', JSON.stringify({
  ...
  methodology: 'Public sitemap URL patterns only, with robots.txt sitemap discovery, bounded recursive sitemap-index expansion and entity-level deduplication against the ToolScout catalog. Competitor content is not copied or used for editorial conclusions. Repeated URL themes are market-coverage signals and must pass first-party verification and ToolScout quality gates before publication.',
...
console.log(JSON.stringify({ sources: sources.map(x => ({name:x.name,status:x.status,urls:x.urls.length})), suppressedCovered, gaps: gaps.length, top: gaps.slice(0,20) }, null, 2));
