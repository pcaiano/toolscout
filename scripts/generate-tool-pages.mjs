...
const logoUrl=tool=>assets?.[tool.slug]?.url||googleLogo(tool);
...
function render(tool){
  ...
  const relatedHtml=related.length?`<section class="section"><div class="sectionHead"><div><div class="eyebrow">Related tools</div><h2>Compare ${esc(tool.name)} with similar options</h2></div>${canCompare(tool)?`<a class="textLink" href="/compare.html?a=${encodeURIComponent(tool.slug)}&source=tool-profile">Start a new comparison</a>`:''}</div><div class="relatedGrid">${related.map(other=>`<article class="relatedCard"><div class="relatedIdentity">${logoUrl(other)?`<img src="${esc(logoUrl(other))}" alt="" width="34" height="34" loading="lazy" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span class="miniFallback" hidden>${esc(initials(other.name))}</span>`:`<span class="miniFallback">${esc(initials(other.name))}</span>`}<div><a class="relatedName" href="/tools/${encodeURIComponent(other.slug)}">${esc(other.name)}</a><span>${esc(other.category)}</span></div></div><a class="compareBtn" href="/compare.html?a=${encodeURIComponent(tool.slug)}&b=${encodeURIComponent(other.slug)}&source=related-tool">Compare</a></article>`).join('')}</div></section>`:'';
