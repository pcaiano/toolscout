...
if (fs.existsSync(affiliatePath)) {
  ...
  for (const [slug, entry] of Object.entries(affiliate)) {
    ...
    if (entry.enabled && !/[?&](?:ref|referral|affiliate|partner|aff)=/i.test(String(entry.url||''))) failures.push(`data/affiliate.json: ${slug} enabled URL should contain a vendor tracking parameter`);
  }
}
