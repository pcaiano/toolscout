...
walk(ROOT);
const hits=[];
for(const file of files){const rel=path.relative(ROOT,file);if(allowed.has(rel)||rel===path.join('scripts','check-public-host.mjs'))continue;const text=fs.readFileSync(file,'utf8');if(text.includes(legacy))hits.push(rel);}
if(hits.length){console.error('Legacy public hostname found outside approved Worker fallback:',hits.join(', '));process.exit(1);}
console.log('Public hostname guard passed: no legacy hostname in ToolScout public or operational files.');
