...
fs.mkdirSync(path.join(ROOT,'reports'),{recursive:true});fs.writeFileSync(path.join(ROOT,'reports','distribution-queue.json'),JSON.stringify({generatedAt:new Date().toISOString(),count:pages.length,policy:'Only guides that passed the autonomous editorial publication gates are eligible for distribution.',items:pages},null,2)+'\n');
