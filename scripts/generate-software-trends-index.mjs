...
const growthPriority=read('reports/growth-priority.json',{gsc:{siteTotals:{}}});
...
const publicData={
  ...
  scope:'ToolScout first-party observations of public software product changes and ToolScout search visibility. This is not a measure of global market share.',
...
const updateCards=updates.map(item=>`<article class="update"><div class="meta"><span>${esc(item.label||'Product update')}</span><span>${esc(item.publishedAt||'')}</span></div><h3>${esc(item.title)}</h3><p>${esc(item.summary)}</p><div class="actions">${item.articleUrl?`<a href="${esc(item.articleUrl)}">Read ToolScout article</a>`:''}<a href="${esc(item.sourceUrl)}" target="_blank" rel="noopener">Official source</a></div></article>`).join('');
