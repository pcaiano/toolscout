...
if(!supervisor.includes('BACKLINK_ATTEMPT_MIN_24H=4')||!supervisor.includes('BACKLINK_STAGNATION_HOURS=24')||!supervisor.includes('execute_bounded_relevant_authority_routes')||!supervisor.includes('rotate_authority_mix_to_relevant_competitor_gap'))fail('Backlink acquisition lacks bounded, relevance-led supervision.');
...
if(!orchestrator.includes("id:'human-demand-first-v4'")||!orchestrator.includes('allocationPct:{existingDemandSearch:60,authorityVendorNetwork:25,aiAeoDiscovery:10,growthRnd:5}')||!orchestrator.includes("strictVerifiedHumanSessions30d:100")||!orchestrator.includes("externalDemandRemainsPrimary:true"))fail('Human-demand-first acquisition policy or its allocation is missing.');
