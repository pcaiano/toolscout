...
function acquisitionScript(){
  return `<script data-toolscout-visitor-tracker="1">(function(){try{var key='toolscout_visitor_v1',ttl=45*24*60*60*1000,now=Date.now(),saved=null;try{saved=JSON.parse(localStorage.getItem(key)||'null')}catch(e){}var valid=saved&&typeof saved.id==='string'&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(saved.id)&&now-Number(saved.lastSeen||0)<ttl;if(!valid&&!window.crypto?.randomUUID)return;var id=valid?saved.id:crypto.randomUUID();try{localStorage.setItem(key,JSON.stringify({id:id,lastSeen:now}))}catch(e){return}function clean(v,n){return String(v||'').replace(/[^A-Za-z0-9._:&=/-]/g,'').slice(0,n||100)}var source='direct',refHost=null;try{var q=new URLSearchParams(location.search),utm=['utm_source','utm_medium','utm_campaign'].map(function(k){var z=clean(q.get(k));return z?k+'='+z:''}).filter(Boolean).join('&'),markers=['ts_action','ts_growth','ts_channel'].map(function(k){var z=clean(q.get(k));return z?k+'='+z:''}).filter(Boolean).join('&'),base='';if(utm)base=utm;else if(q.get('source'))base=clean(q.get('source'));else if(document.referrer){var u=new URL(document.referrer),h=u.hostname.replace(/^www\\./,'');if(h&&h!==location.hostname){refHost=h.slice(0,120);base=clean('ref:'+h)}}if(base||markers)source=clean([base,markers].filter(Boolean).join('&'),300)}catch(e){}var sessionKey='toolscout_session_v2',sessionTtl=30*60*1000,sessionSaved=null;try{sessionSaved=JSON.parse(localStorage.getItem(sessionKey)||'null')}catch(e){}var sessionValid=sessionSaved&&typeof sessionSaved.id==='string'&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(sessionSaved.id)&&now-Number(sessionSaved.lastSeen||0)<sessionTtl;var sessionId=sessionValid?sessionSaved.id:crypto.randomUUID();try{localStorage.setItem(sessionKey,JSON.stringify({id:sessionId,lastSeen:now}))}catch(e){}try{document.cookie='toolscout_session='+encodeURIComponent(sessionId)+'; Max-Age=1800; Path=/; SameSite=Lax; Secure'}catch(e){}function postFunnel(type){return fetch('/api/events',{method:'POST',credentials:'same-origin',keepalive:true,headers:{'content-type':'application/json'},body:JSON.stringify({event_id:'evt_'+crypto.randomUUID(),session_id:sessionId,event_type:type,path:location.pathname.slice(0,200)||'/',source:source,referrer_host:refHost})})}function confirmBrowserSession(){var startedKey='toolscout_started_'+sessionId,started=false;try{started=sessionStorage.getItem(startedKey)==='1'}catch(e){}if(!started){try{sessionStorage.setItem(startedKey,'1')}catch(e){}postFunnel('session_started').catch(function(){})}var confirmedKey='toolscout_page_confirmed_v1',confirmed=false;try{confirmed=localStorage.getItem(confirmedKey)===sessionId}catch(e){}if(!confirmed){postFunnel('page_confirmed').then(function(r){if(r&&r.ok){try{localStorage.setItem(confirmedKey,sessionId)}catch(e){}}}).catch(function(){})}}function activateBrowserSession(){if(document.visibilityState==='visible'){requestAnimationFrame(function(){setTimeout(confirmBrowserSession,0)})}else{var onVisible=function(){if(document.visibilityState==='visible'){document.removeEventListener('visibilitychange',onVisible);requestAnimationFrame(confirmBrowserSession)}};document.addEventListener('visibilitychange',onVisible)}}if(document.prerendering){document.addEventListener('prerenderingchange',activateBrowserSession,{once:true})}else activateBrowserSession();fetch('/api/visitor',{method:'POST',credentials:'same-origin',keepalive:true,headers:{'content-type':'application/json'},body:JSON.stringify({visitor_id:id,path:location.pathname.slice(0,200)||'/',source:source,referrer_host:refHost})}).catch(function(){})}catch(e){}})();</script>`;
}
