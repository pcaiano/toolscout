...
The callback accepts only the ToolScout owner Google account. The refresh token is never sent to the browser after the callback and is never stored in repository files.

OAuth connection state is stored in `google_oauth_connections`, created by migration `0081_google_analytics_oauth.sql`. The Worker can also create the table idempotently at first connection if the migration has not yet run.
...
The legacy `/analytics/api/ga4-health` route is blocked at the top Worker layer. GA4 connection state is exposed only inside the protected Command Center data path.
