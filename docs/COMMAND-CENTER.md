...
Visitor geography is now exposed in the Human Sessions traffic detail for Today and Last 24h. It uses the Cloudflare country code already captured on Browser Guard allowed sessions and joins it only to browser-confirmed visitor records. Raw IP addresses are not stored. Country is network location and can be affected by VPNs or proxies. This change does not reset or recreate sessions or visitors.
