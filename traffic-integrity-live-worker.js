...
function monetizationProofPanel(){return `<section id="ts-monetization-proof" data-toolscout-monetization-proof="1" style="max-width:1180px;margin:24px auto;padding:0 18px;font-family:Inter,system-ui,sans-serif"><div style="background:#fff;border:1px solid #e4e7ec;border-radius:18px;padding:20px"><div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#667085">Monetization proof</div><h2 style="margin:6px 0 4px;font-size:22px;color:#101828">Revenue evidence and unit economics</h2><p style="margin:0;color:#667085;font-size:13px">Revenue is vendor-confirmed only. Clicks do not imply revenue.</p></div><div id="ts-mp-state" style="font-size:12px;font-weight:750;color:#344054">Loading</div></div><div id="ts-mp-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:16px"></div></div></section><script data-toolscout-monetization-proof-script="1">(function(){function money(v,c){if(v===null||v===undefined||!Number.isFinite(Number(v)))return 'Unknown';try{return new Intl.NumberFormat('en',{style:'currency',currency:c||'EUR',maximumFractionDigits:2}).format(Number(v))}catch(e){return String(Number(v).toFixed(2))}}function num(v){return Number.isFinite(Number(v))?new Intl.NumberFormat('en').format(Number(v)):'Unknown'}function card(label,value){return '<div style="background:#f8fafc;border:1px solid #eaecf0;border-radius:12px;padding:13px"><div style="font-size:11px;color:#667085;text-transform:uppercase;letter-spacing:.05em">'+label+'</div><div style="font-size:20px;font-weight:800;color:#101828;margin-top:5px">'+value+'</div></div>'}fetch('/analytics/api/stats',{credentials:'same-origin',cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('stats');return r.json()}).then(function(d){var p=d.monetizationProof||{},c=p.currency||'EUR';document.getElementById('ts-mp-state').textContent=String(p.sampleState||p.status||'unknown').replace(/-/g,' ');document.getElementById('ts-mp-grid').innerHTML=card('Confirmed affiliate revenue',money(p.confirmedRevenue,c))+card('Monetized outbound clicks',num(p.monetizedOutboundClicks))+card('Earnings per monetized click',money(p.earningsPerMonetizedClick,c))+card('Revenue per 1,000 human sessions',money(p.revenuePer1000HumanSessions,c))+card('Confirmed conversions',num(p.confirmedConversions))+card('Human sessions',num(p.humanSessions))}).catch(function(){var s=document.getElementById('ts-mp-state');if(s)s.textContent='Unavailable'})})();</script>`}
...
async function decorate(response,isAnalytics,pathname){
  ...
  if(isAnalytics&&!html.includes('data-toolscout-monetization-proof="1"'))html=html.replace(/<\/body>/i,monetizationProofPanel()+'</body>');
