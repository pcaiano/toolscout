...
async function visitorSnapshot(env){
  ...
  const rows=await env.DB.prepare(`SELECT visitor_id,session_id,created_at FROM confirmed_visitor_events WHERE created_at>=? ORDER BY created_at ASC`).bind(String(meta?.value||'')).all();
  const now=new Date(),todayKey=dayKey(now),monthPrefix=todayKey.slice(0,7),cutoff=now.getTime()-86400000;
  const last24=new Set(),today=new Set(),month=new Set(),all=new Set(),sessions24=new Set(),sessionsToday=new Set(),dailySets=new Map();
  for(const row of rows?.results||[]){
    const at=parseUtc(row.created_at),vid=String(row.visitor_id||''),sid=String(row.session_id||'');if(!at||!vid)continue;
    const d=dayKey(at);all.add(vid);if(!dailySets.has(d))dailySets.set(d,new Set());dailySets.get(d).add(vid);
    if(at.getTime()>=cutoff){last24.add(vid);if(sid)sessions24.add(sid)}
    if(d===todayKey){today.add(vid);if(sid)sessionsToday.add(sid)}
    if(d.startsWith(monthPrefix))month.add(vid);
  }
  const trackingDay=dayKey(trackingSince),coverage={last24Complete:trackingSince.getTime()<=cutoff,todayComplete:trackingDay<todayKey,monthToDateComplete:trackingDay.slice(0,7)<monthPrefix};
  const [guard24,guardToday]=await Promise.all([
    env.DB.prepare(`SELECT COUNT(DISTINCT session_id) count FROM traffic_guard_events WHERE decision='allowed' AND created_at>=datetime('now','-24 hours')`).first(),
    env.DB.prepare(`SELECT session_id,created_at FROM traffic_guard_events WHERE decision='allowed' AND created_at>=datetime('now','-36 hours')`).all()
  ]);
  const guardTodaySet=new Set();for(const row of guardToday?.results||[]){const at=parseUtc(row.created_at);if(at&&dayKey(at)===todayKey)guardTodaySet.add(String(row.session_id||''))}
  const daily=[];const p=zonedParts(now),y=Number(p.year),m=Number(p.month),d=Number(p.day);
  for(let offset=-29;offset<=0;offset++){const probe=new Date(Date.UTC(y,m-1,d+offset,12,0,0)),key=dayKey(probe);daily.push({day:key,visitors:dailySets.get(key)?.size||0})}
  return {
    ...
    last24:last24.size,today:today.size,monthToDate:month.size,sinceTracking:all.size,
    coverage,daily,dailyAverageMTD:null,projectedMonth:null,
    integrity:{guardSessionsLast24:Number(guard24?.count||0),linkedSessionsLast24:sessions24.size,guardSessionsToday:guardTodaySet.size,linkedSessionsToday:sessionsToday.size}
  };
}
