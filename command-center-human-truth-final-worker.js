...
function renderFinal(d){
  ...
  root.innerHTML=chart(trend.points||[])+tabs+'<div class="tsTruthHero"><div><small>'+label(selected)+'</small><b>'+fmt(sessionValue(latest,selected))+'</b><span>Strict verified human sessions</span></div><div>'+pill(ok)+'</div></div><div class="tsTruthForecast"><small>Projected human sessions this month</small><b>'+(proj==null?'Unavailable':fmt(proj))+'</b><span>Projected from the strict verified human month-to-date session pace</span></div><div class="note" style="margin-top:10px"><strong>Canonical definition:</strong> one session with positive strict-human evidence in D1. Accepted evidence is trusted interaction or verified first-party outbound navigation. Browser validation alone is diagnostic. Owner, known bots and synthetic traffic are excluded.</div>';
}
function install(){
