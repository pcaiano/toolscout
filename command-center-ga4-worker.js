...
async function ga4Snapshot(env){
  ...
  if(!cfg.clientEmail||!cfg.privateKey)return {status:'unavailable',canonical:true,source:'Google Analytics 4 Data API',reason:'GA4 service account credentials are not configured in the ToolScout Worker.',measurementId:cfg.measurementId,fetchedAt:new Date().toISOString()};
  try{
    const token=await accessToken(env),propertyId=await resolvePropertyId(env,token),now=new Date(),local=zoneParts(now),today=ymd(local),monthStart=`${local.year}-${String(local.month).padStart(2,'0')}-01`,yesterdayDate=new Date(now.getTime()-36*3600000),yesterday=ymd(zoneParts(yesterdayDate)),cutoff=compactMinute(new Date(now.getTime()-24*3600000)),current=compactMinute(now);
    ...
    const sessionsToday=firstMetric(todayReport,0),mtd=firstMetric(mtdReport,0),elapsedDays=Math.max(1,local.day),dailyAverage=mtd/elapsedDays,projection=dailyAverage*daysInMonth(local.year,local.month),timeZone=todayReport?.metadata?.timeZone||mtdReport?.metadata?.timeZone||BUSINESS_TIME_ZONE;
    return {status:'connected',canonical:true,source:'Google Analytics 4 Data API',propertyId,measurementId:cfg.measurementId,timeZone,sessions:{today:sessionsToday,last24Hours,monthToDate:mtd,dailyAverageMTD:Number(dailyAverage.toFixed(2)),projectedMonth:Math.round(projection)},users:{today:firstMetric(todayReport,1),activeToday:firstMetric(todayReport,2),monthToDate:firstMetric(mtdReport,1)},sources:sourceRows(sourcesReport),fetchedAt:new Date().toISOString(),consentNote:'GA4 acquisition is consent dependent under the current ToolScout consent implementation. These figures reproduce the GA4 reporting population and are not expanded with ToolScout traffic classification estimates.'};
  }catch(error){return {status:'unavailable',canonical:true,source:'Google Analytics 4 Data API',reason:String(error?.message||error),measurementId:cfg.measurementId,fetchedAt:new Date().toISOString()}}
}
...
function acquisitionScript(){return `<script data-ga4-acquisition-renderer="v1">(function(){
...
function draw(d){
  const a=d&&d.acquisition||{},c=d&&d.commerceTruth||{},q=d&&d.trafficQuality||{},root=document.getElementById('acquisitionTruthBody'),meta=document.getElementById('acquisitionTruthMeta');if(!root)return;
  if(a.status!=='connected'){
    if(meta)meta.textContent='GA4 unavailable';root.innerHTML='<div class="bug"><b>GA4 acquisition is unavailable.</b><div style="margin-top:6px">'+esc2(a.reason||'Configure the GA4 reporting credentials in the ToolScout Worker.')+'</div></div>'+row2('Traffic Quality diagnostic',num2(q.strictHumanSessionsLast24Hours)+' / 24h','Diagnostic only. It is not promoted to canonical acquisition.')+(c.status==='connected'?row2('Server outbound / 24h',num2(c.last24Hours&&c.last24Hours.outbound),num2(c.last24Hours&&c.last24Hours.monetized)+' monetized'):'');
  }else{
    const s=a.sessions||{},m=c.monthToDate||{},h=c.last24Hours||{},ratio=s.monthToDate?m.outbound/s.monthToDate*100:null;if(meta)meta.textContent='GA4 property '+a.propertyId+' · '+(a.timeZone||'property timezone');
...
async function decoratePage(response){
  ...
  if(!html.includes('data-ga4-acquisition-renderer="v1"'))html=html.replace('</body>',acquisitionScript()+'</body>');
