...
export default{
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&u.pathname==='/api/runtime/e2e-reconcile-20260923-owner-authorized'){
      if(!env.ADMIN_TOKEN)return Response.json({ok:false,error:'admin_token_unavailable'},{status:503});
      const internal=new Request(new URL('/api/growth/supervisor/audit',request.url),{method:'POST',headers:{Authorization:`Bearer ${env.ADMIN_TOKEN}`,'Content-Type':'application/json'}});
