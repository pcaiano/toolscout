...
(function(){
  const esc2=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#039;'}[m]));
  ...
  function initialLoad(){
    setTimeout(refreshAffiliateStatus,250);
    const refresh=document.getElementById('refresh');
    if(refresh&&!window.__toolscoutInitialRefresh){window.__toolscoutInitialRefresh=true;setTimeout(()=>refresh.click(),60)}
  }
...
export default {
  ...
  async fetch(request,env,ctx){
    ...
    // Keep the canonical standalone Affiliate Coverage Status block generated by the V2 worker.
    // Also add the compact in-card detail as a secondary view for quick scanning.
