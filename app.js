...
const questions = [
  { id: 'goal', title: 'What are you mainly trying to do?', choices: [
    ['crm', 'Manage customers and sales'], ['marketing', 'Marketing and automation'], ['seo', 'Improve SEO and search visibility'], ['forms', 'Collect information with forms']
  ]},
  { id: 'budget', title: 'What is your monthly budget?', choices: [
    ['free', 'Free only'], ['low', 'Under $25/month'], ['mid', '$25–100/month'], ['high', '$100+/month']
  ]},
  { id: 'team', title: 'Who will use the tool?', choices: [
    ['solo', 'Just me'], ['small', '2–5 people'], ['team', '6–20 people'], ['large', '20+ people']
  ]},
  { id: 'priority', title: 'What matters most?', choices: [
    ['ease', 'Easy to use'], ['automation', 'Automation'], ['integrations', 'Integrations'], ['features', 'Advanced features']
  ]}
];

async function boot() {
  const [toolsResponse, intentsResponse] = await Promise.all([
    fetch('/data/tools.json', { cache: 'no-store' }), fetch('/data/intents.json', { cache: 'no-store' })
  ]);
...
function detectIntent(query) {
  const q = normalize(query); let best = null; let bestScore = 0;
...
function scoreTool(tool, query, intent, profile = {}) {
  const q = normalize(query); const words = tokenize(query);
...
function trackClick(tool, intent, profile = {}) {
  const event = { tool: tool.slug, intent: intent?.slug || 'general', profile, timestamp: new Date().toISOString() };
  const clicks = JSON.parse(localStorage.getItem('toolscout_clicks') || '[]');
  clicks.push(event); localStorage.setItem('toolscout_clicks', JSON.stringify(clicks.slice(-500)));
}

function renderResults(query, profile = {}) {
  const intent = detectIntent(query); const results = state.tools.map((tool) => ({ ...tool, score: scoreTool(tool, query, intent, profile) })).sort((a,b) => b.score-a.score).slice(0,3);
...
function showQuestion() {
  const q = questions[state.step]; document.getElementById('guided').style.display='block'; document.getElementById('progress').textContent=`Question ${state.step+1} of ${questions.length}`; document.getElementById('question').textContent=q.title;
  ...
  document.querySelectorAll('.choice').forEach(btn=>btn.addEventListener('click',()=>{state.answers[q.id]=btn.dataset.value;if(state.step<questions.length-1){state.step++;showQuestion();}else{document.getElementById('guided').style.display='none';renderResults('guided recommendation',{...state.answers});}}));
}
