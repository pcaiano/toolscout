...
async function execute(request,env){
  const p=await cfg(request,env),max=Math.max(1,Math.min(12,Number(p.policy?.max_automatic_per_run)||12)),maxAttempts=Math.max(1,Math.min(5,Number(p.policy?.max_attempts)||3));
