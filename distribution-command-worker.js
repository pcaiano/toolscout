...
async function ops(env){const [subs,editorial,assets,learned,events]=await Promise.all([
 q(env,`SELECT status,COUNT(*) n FROM distribution_submissions GROUP BY status`),
 ...
 return {status:'connected',assetsKnown:Number(assets?.assets||0),lastAssetSeenAt:assets?.last_seen||null,submissions:{ready:s.ready||0,submitted:s.submitted||0,failed:s.failed||0,humanRequired:s.human_required||0,adapterMissing:s.adapter_missing||0},editorial:{prepared:e.prepared||0,published:e.published||0,skipped:e.skipped||0},learning:{surfaces:Number(learned?.learned||0),averagePerformance:Number(Number(learned?.avg_performance||0).toFixed(1)),lastLearnedAt:learned?.learned_at||null},events:{count:Number(events?.events||0),lastEventAt:events?.last_event||null}};
}
