...
async function ops(env){
  const [
    ...
    indexNowSubmitted,indexNowDelivery,pendingReview,operating,autonomy30d,placements,networkStates
  ]=await Promise.all([
    ...
    one(env,`SELECT COUNT(*) backlog,COALESCE(SUM(CASE WHEN ds.status='failed' AND ds.error LIKE 'retryable:indexnow_%' THEN 1 ELSE 0 END),0) rate_limited,MAX(st.retry_after_at) retry_after_at,MAX(st.last_network_attempt_at) last_network_attempt_at,MAX(st.rate_limit_count) max_rate_limit_count FROM distribution_submissions ds LEFT JOIN distribution_delivery_state st ON st.submission_id=ds.submission_id WHERE ds.surface_slug='indexnow' AND ds.status IN ('ready','failed')`),
    ...
    one(env,`SELECT COUNT(*) attempted,SUM(CASE WHEN status='submitted' THEN 1 ELSE 0 END) submitted FROM distribution_submissions WHERE human_required=0 AND attempts>0 AND created_at>=datetime('now','-30 days')`),
    one(env,`SELECT COUNT(*) placements,COALESCE(SUM(backlink_verified),0) backlinks FROM distribution_placements WHERE placement_verified=1`),
    q(env,`SELECT status,COUNT(*) n FROM distribution_network_outreach GROUP BY status`)
  ]);
  const map=r=>Object.fromEntries((r.results||[]).map(x=>[x.status,Number(x.n||0)]));
  const s=map(subs),e=map(editorial),o=map(operating),nw=map(networkStates);
  ...
  return {
    ...
    submissions:{
      ...
      indexNowBacklog:Number(indexNowDelivery?.backlog||0),
      indexNowRateLimited:Number(indexNowDelivery?.rate_limited||0),
      indexNowRetryAfterAt:indexNowDelivery?.retry_after_at||null,
      indexNowLastNetworkAttemptAt:indexNowDelivery?.last_network_attempt_at||null,
      indexNowMaxRateLimitCount:Number(indexNowDelivery?.max_rate_limit_count||0),
    ...
    editorial:{prepared:e.prepared||0,autonomyPending:e.autonomy_pending||0,published:e.published||0,skipped:e.skipped||0},
    autonomy:{attempted30d:Number(autonomy30d?.attempted||0),submitted30d:Number(autonomy30d?.submitted||0),verifiedPlacements:Number(placements?.placements||0),verifiedBacklinks:Number(placements?.backlinks||0)},
    network:{queued:nw.queued||0,contactFound:nw.contact_found||0,sent:nw.sent||0,adopted:nw.adopted||0,suppressed:nw.suppressed_no_contact||0,sendFailed:nw.send_failed||0},
...
function inject(html){if(html.includes('id="distributionOpsFinal"'))return html;const block=`<section class="section" id="distributionOpsFinal"><div class="sectionHead"><h2>Distribution Operations</h2><span>End-to-end engine health</span></div><div class="grid4" id="distributionOpsCards"></div><div class="panel section"><div class="sectionHead"><h2>Engine lifecycle</h2><span>Discover → Score → Package → Distribute → Amplify → Measure → Learn</span></div><div id="distributionOpsLifecycle" class="note">Refresh to load.</div></div></section>`;const js=`<script>(function(){function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(m){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]})}function card(a,b,c){return '<div class="card"><small>'+esc(a)+'</small><b>'+esc(b)+'</b><span>'+esc(c)+'</span></div>'}function render(d){var x=d&&d.distributionOperations;if(!x)return;var s=x.submissions||{},e=x.editorial||{},m=x.embeds||{},a=x.agentProtocols||{},mc=a.mcp||{},aa=a.a2a||{},o=x.operating||{},z=x.autonomy||{},nw=x.network||{};var retry=s.indexNowRetryAfterAt?('Next retry '+s.indexNowRetryAfterAt+' UTC'):'No retry scheduled';var c=document.getElementById('distributionOpsCards');if(c)c.innerHTML=card('Assets known',x.assetsKnown,'Event-driven distribution state')+card('Scale',o.scale||0,'Strong evidence, highest operating priority')+card('Measure',o.measure||0,'Directional evidence or committed experiment')+card('Explore',o.explore||0,'Bounded discovery of free surfaces')+card('Suspend',o.suspend||0,'Blocked or insufficient-return surfaces')+card('Autonomous attempts',z.attempted30d||0,'Verified automatic execution attempts in the last 30 days')+card('Submitted surfaces',s.submittedSurfaces||0,'Distinct external surfaces with accepted submissions')+card('Public live / verified',s.publicVerified,'Confirmed third-party public destinations')+card('Verified backlinks',z.verifiedBacklinks||0,'Public placements where ToolScout link is confirmed')+card('Network publishers',nw.adopted||0,'Publishers with verified embed, click or backlink adoption')+card('Publisher outreach',nw.sent||0,(nw.contactFound||0)+' ready contacts · '+(nw.queued||0)+' researching')+card('Pending review',s.pendingReview||0,'Submitted surfaces awaiting editorial approval')+card('IndexNow URLs',s.indexNowSubmitted||0,'Distinct URL submission records accepted by IndexNow')+card('IndexNow deferred',s.indexNowRateLimited||0,retry)+card('Machine discovery',s.machineDiscovery,'Live first-party agent/API discovery surfaces')+card('MCP calls',mc.calls||0,(mc.successful||0)+' successful · '+(mc.recommendations||0)+' recommendations')+card('A2A messages',aa.calls||0,(aa.successful||0)+' successful · '+(aa.recommendations||0)+' recommendations')+card('Automation gaps',s.adapterMissing,'Ready surfaces awaiting verified adapters')+card('Research required',s.researchRequired,'Candidates awaiting route verification')+card('Auth required',s.authRequired,'One-time credential or identity setup')+card('Setup required',s.setupRequired,'Metadata, profile or media prerequisites')+card('Approval required',s.approvalRequired,'Provider or paid policy requires explicit approval')+card('Policy blocked',s.policyBlocked,'Paid, reciprocal or disallowed by policy')+card('Human required',s.humanRequired||0,'Only proven human-only gates such as CAPTCHA or explicit approval')+card('Embed publishers',m.publishers||0,(m.impressions||0)+' impressions · '+(m.clicks||0)+' clicks')+card('Learning surfaces',x.learning&&x.learning.surfaces,'Avg performance '+(x.learning&&x.learning.averagePerformance||0));var l=document.getElementById('distributionOpsLifecycle');if(l)l.innerHTML='Closed loop: discover → qualify route → execute automatically → verify public placement/backlink → measure traffic → learn → replicate.  Operating state: '+esc(o.scale||0)+' scale · '+esc(o.measure||0)+' measure · '+esc(o.explore||0)+' explore · '+esc(o.suspend||0)+' suspend. IndexNow backlog: '+esc(s.indexNowBacklog||0)+' · rate limited: '+esc(s.indexNowRateLimited||0)+' · '+esc(retry)+'. Embed installations: '+esc(m.installations||0)+' · MCP last seen: '+esc(mc.lastSeenAt||'n/a')+' · A2A last seen: '+esc(aa.lastSeenAt||'n/a')+' · last engine event: '+esc(x.events&&x.events.lastEventAt||'n/a');}var f=window.fetch;window.fetch=async function(){var r=await f.apply(this,arguments);try{var u=String(arguments[0]&&arguments[0].url||arguments[0]||'');if(u.indexOf('/api/stats')!==-1)r.clone().json().then(render).catch(function(){})}catch(e){}return r};})();</script>`;return html.replace('</body>',block+js+'</body>');}
