export default {
  ...
  async scheduled(event,env,ctx){ctx.waitUntil(Promise.all([env.DB.prepare("DELETE FROM click_events WHERE created_at < datetime('now','-180 days')").run(),env.DB.prepare("DELETE FROM search_events WHERE created_at < datetime('now','-180 days')").run()]));}
};