export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/content-signals' && request.method === 'GET') {
      try {
        ...
        return Response.json({ok:true,windowDays:30,signals:(rows.results||[]).map(r=>({intent_slug:String(r.intent_slug),searches:Number(r.searches||0)}))},{headers:cors});
      } catch { return Response.json({ok:false,signals:[]},{status:200,headers:cors}); }
    }
    ...
    if (url.pathname === '/api/opportunities/refresh' && request.method === 'POST') {
      ...
      try {
        ...
        const known=new Set((catalog||[]).map(item=>String(item.slug||'')));
        const inferObservedProfile=slug=>{const s=String(slug||'').toLowerCase();if(/crm|customer|sales|pipeline/.test(s))return{category:'crm',weights:{category:35,sales:25,automation:15,integrations:15,ease:10}};if(/seo|keyword|search/.test(s))return{category:'seo',weights:{category:35,seo:25,research:15,content:10,integrations:10,ease:5}};if(/marketing|email|advertis|content/.test(s))return{category:'marketing',weights:{category:30,marketing:25,automation:20,content:10,integrations:10,ease:5}};if(/automation|workflow|integrat/.test(s))return{category:'automation',weights:{automation:30,integrations:30,ease:15,category:15,price:10}};if(/form|survey|lead-capture/.test(s))return{category:'forms',weights:{category:35,integrations:20,ease:20,automation:15,price:10}};if(/agency|agencies|client/.test(s))return{category:'business',weights:{agency:25,automation:25,integrations:25,marketing:10,ease:15}};return{category:'general',weights:{}}};
        const observed=(rows.results||[]).map(r=>String(r.intent_slug)).filter(slug=>slug&&slug!=='general'&&!known.has(slug));
        const intents=[...(catalog||[]),...observed.map(slug=>{const p=inferObservedProfile(slug);return{slug,category:p.category,weights:p.weights}})];
        ...
        for(const item of intents) {
          ...
          const profile=profiles?.[intent]||item;
          ...
          const top=ranked.slice(0,3); const catalogFit=top.length&&profile?.weights&&Object.keys(profile.weights).length?Math.min(20,(top.reduce((a,b)=>a+b,0)/top.length)*2):0;
          const demand=Math.min(50,sessions*5), commercial=/(crm|seo|marketing|agency|agencies|automation|lead|sales|email|project)/i.test(intent)?25:10, duplication=0, score=Math.max(0,Math.min(100,demand+commercial+catalogFit-duplication));
          const status=pageExists?'published':(score>=50?'ready':'candidate');
  ...
  async scheduled(event,env,ctx){
    const run=async()=>{
      ...
      const catalog=await (await env.ASSETS.fetch(new Request(new URL('/data/intents.json','https://trytoolscout.org')))).json();
      const profiles=await (await env.ASSETS.fetch(new Request(new URL('/data/intent-profiles.json','https://trytoolscout.org')))).json();
      const tools=await (await env.ASSETS.fetch(new Request(new URL('/data/tools.json','https://trytoolscout.org')))).json();
      const rows=await env.DB.prepare("SELECT intent_slug,COUNT(DISTINCT session_id) AS search_sessions FROM search_events WHERE created_at >= datetime('now','-30 days') GROUP BY intent_slug").all();
      const demandMap=new Map((rows.results||[]).map(r=>[String(r.intent_slug),Number(r.search_sessions||0)]));
      const known=new Set((catalog||[]).map(item=>String(item.slug||'')));
      const inferObservedProfile=slug=>{const s=String(slug||'').toLowerCase();if(/crm|customer|sales|pipeline/.test(s))return{category:'crm',weights:{category:35,sales:25,automation:15,integrations:15,ease:10}};if(/seo|keyword|search/.test(s))return{category:'seo',weights:{category:35,seo:25,research:15,content:10,integrations:10,ease:5}};if(/marketing|email|advertis|content/.test(s))return{category:'marketing',weights:{category:30,marketing:25,automation:20,content:10,integrations:10,ease:5}};if(/automation|workflow|integrat/.test(s))return{category:'automation',weights:{automation:30,integrations:30,ease:15,category:15,price:10}};if(/form|survey|lead-capture/.test(s))return{category:'forms',weights:{category:35,integrations:20,ease:20,automation:15,price:10}};if(/agency|agencies|client/.test(s))return{category:'business',weights:{agency:25,automation:25,integrations:25,marketing:10,ease:15}};return{category:'general',weights:{}}};
      ...
      const intents=[...(catalog||[]),...observed.map(slug=>{const p=inferObservedProfile(slug);return{slug,category:p.category,weights:p.weights}})];
      const scoreTool=(tool,item,profile)=>{const weights=profile?.weights||item?.weights||{};let total=0,weight=0;for(const [key,w] of Object.entries(weights)){const n=Number(w)||0;let value=0;if(key==='category')value=String(tool.category||'').toLowerCase()===String(item.category||'').toLowerCase()?10:0;else if(key==='freePlan')value=tool.freePlan?10:0;else if(key==='simplicity'||key==='ease')value=Number(tool.scores?.ease||0);else value=Number(tool.scores?.[key]||0);total+=value*n;weight+=n;}return weight?total/weight:0;};
      for(const item of intents){const intent=String(item.slug||'').slice(0,100);if(!intent)continue;const sessions=demandMap.get(intent)||0,profile=profiles?.[intent]||item;let pageExists=false;try{pageExists=(await env.ASSETS.fetch(new Request(new URL('/'+intent+'.html','https://trytoolscout.org')))).ok;}catch{}const ranked=(tools||[]).map(t=>scoreTool(t,item,profile)).sort((a,b)=>b-a),top=ranked.slice(0,3),catalogFit=top.length&&profile?.weights&&Object.keys(profile.weights).length?Math.min(20,(top.reduce((a,b)=>a+b,0)/top.length)*2):0,demand=Math.min(50,sessions*5),commercial=/(crm|seo|marketing|agency|agencies|automation|lead|sales|email|project)/i.test(intent)?25:10,duplication=0,score=Math.max(0,Math.min(100,demand+commercial+catalogFit-duplication)),status=pageExists?'published':(score>=50?'ready':'candidate');await env.DB.prepare("INSERT INTO seo_opportunities (intent_slug,search_sessions,commercial_score,catalog_score,duplication_penalty,opportunity_score,status,updated_at) VALUES (?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(intent_slug) DO UPDATE SET search_sessions=excluded.search_sessions,commercial_score=excluded.commercial_score,catalog_score=excluded.catalog_score,duplication_penalty=excluded.duplication_penalty,opportunity_score=excluded.opportunity_score,status=excluded.status,updated_at=datetime('now')").bind(intent,sessions,commercial,catalogFit,duplication,score,status).run();}
    };
