export default {
  ...
  async scheduled(event,env,ctx){
    const run=async()=>{
      ...
      const intents=[...(catalog||[]),...observed.map(slug=>{const p=inferObservedProfile(slug);return{slug,category:p.category,weights:p.weights}})];
      const scoreTool=(tool,item,profile)=>{const weights=profile?.weights||item?.weights||{};let total=0,weight=0;for(const [key,w] of Object.entries(weights)){const n=Number(w)||0;let value=0;if(key==='category')value=String(tool.category||'').toLowerCase()===String(item.category||'').toLowerCase()?10:0;else if(key==='freePlan')value=tool.freePlan?10:0;else if(key==='simplicity'||key==='ease')value=Number(tool.scores?.ease||0);else value=Number(tool.scores?.[key]||0);total+=value*n;weight+=n;}return weight?total/weight:0;};
      for(const item of intents){const intent=String(item.slug||'').slice(0,100);if(!intent)continue;const sessions=demandMap.get(intent)||0,profile=profiles?.[intent]||item;let pageExists=false;try{pageExists=(await env.ASSETS.fetch(new Request(new URL('/'+intent+'.html','https://toolscout.luxurybuyerintelligence.workers.dev')))).ok;}catch{}const ranked=(tools||[]).map(t=>scoreTool(t,item,profile)).sort((a,b)=>b-a),top=ranked.slice(0,3),catalogFit=top.length&&profile?.weights&&Object.keys(profile.weights).length?Math.min(20,(top.reduce((a,b)=>a+b,0)/top.length)*2):0,demand=Math.min(50,sessions*5),commercial=/(crm|seo|marketing|agency|agencies|automation|lead|sales|email|project)/i.test(intent)?25:10,duplication=0,score=Math.max(0,Math.min(100,demand+commercial+catalogFit-duplication)),status=pageExists?'published':(score>=50?'ready':'candidate');await env.DB.prepare("INSERT INTO seo_opportunities (intent_slug,search_sessions,commercial_score,catalog_score,duplication_penalty,opportunity_score,status,updated_at) VALUES (?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(intent_slug) DO UPDATE SET search_sessions=excluded.search_sessions,commercial_score=excluded.commercial_score,catalog_score=excluded.catalog_score,duplication_penalty=excluded.duplication_penalty,opportunity_score=excluded.opportunity_score,status=excluded.status,updated_at=datetime('now')").bind(intent,sessions,commercial,catalogFit,duplication,score,status).run();}
    };
