export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/opportunities/refresh' && request.method === 'POST') {
      ...
      try {
        ...
        const intents=[...(catalog||[]),...observed.map(slug=>({slug,category:'general',weights:{}}))];
        ...
        for(const item of intents) {
          ...
          const top=ranked.slice(0,3); const catalogFit=top.length&&profile?Math.min(20,(top.reduce((a,b)=>a+b,0)/top.length)*2):0;
  ...
  async scheduled(event,env,ctx){
    const run=async()=>{
      ...
      const rows=await env.DB.prepare("SELECT intent_slug,COUNT(DISTINCT session_id) AS search_sessions FROM search_events WHERE created_at >= datetime('now','-30 days') GROUP BY intent_slug").all();
      const demandMap=new Map((rows.results||[]).map(r=>[String(r.intent_slug),Number(r.search_sessions||0)]));
      const known=new Set((catalog||[]).map(item=>String(item.slug||''))); const intents=[...(catalog||[]),...(rows.results||[]).map(r=>String(r.intent_slug)).filter(slug=>slug&&slug!=='general'&&!known.has(slug)).map(slug=>({slug,category:'general',weights:{}}))];
      const scoreTool=(tool,item,profile)=>{const weights=profile?.weights||item?.weights||{};let total=0,weight=0;for(const [key,w] of Object.entries(weights)){const n=Number(w)||0;let value=0;if(key==='category')value=String(tool.category||'').toLowerCase()===String(item.category||'').toLowerCase()?10:0;else if(key==='freePlan')value=tool.freePlan?10:0;else if(key==='simplicity'||key==='ease')value=Number(tool.scores?.ease||0);else value=Number(tool.scores?.[key]||0);total+=value*n;weight+=n;}return weight?total/weight:0;};
      for(const item of intents){const intent=String(item.slug||'').slice(0,100);if(!intent)continue;const sessions=demandMap.get(intent)||0,profile=profiles?.[intent]||null;let pageExists=false;try{pageExists=(await env.ASSETS.fetch(new Request(new URL('/'+intent+'.html','https://toolscout.luxurybuyerintelligence.workers.dev')))).ok;}catch{}const ranked=(tools||[]).map(t=>scoreTool(t,item,profile)).sort((a,b)=>b-a),top=ranked.slice(0,3),catalogFit=top.length&&profile?Math.min(20,(top.reduce((a,b)=>a+b,0)/top.length)*2):0,demand=Math.min(50,sessions*5),commercial=/(crm|seo|marketing|agency|agencies|automation|lead|sales|email|project)/i.test(intent)?25:10,score=Math.max(0,Math.min(100,demand+commercial+catalogFit)),status=pageExists?'published':(score>=50?'ready':'candidate');await env.DB.prepare("INSERT INTO seo_opportunities (intent_slug,search_sessions,commercial_score,catalog_score,duplication_penalty,opportunity_score,status,updated_at) VALUES (?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(intent_slug) DO UPDATE SET search_sessions=excluded.search_sessions,commercial_score=excluded.commercial_score,catalog_score=excluded.catalog_score,duplication_penalty=excluded.duplication_penalty,opportunity_score=excluded.opportunity_score,status=excluded.status,updated_at=datetime('now')").bind(intent,sessions,commercial,catalogFit,0,score,status).run();}
    }; ctx.waitUntil(run());
  }
};
