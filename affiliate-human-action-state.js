...
export async function recordAffiliateHumanAction(env,input={}){
  ...
  let targetState=EVENT_STATE[event];
  if(!toolSlug)return {ok:false,error:'tool_slug_required'};
  if(!targetState&&event!=='completed')return {ok:false,error:'unsupported_event'};
  ...
  if(event==='completed')targetState=existing.submitted_at?'pending_review':'submitted';
  ...
  const submittedAt=['submitted','completed'].includes(event)?(existing.submitted_at||now):existing.submitted_at;
