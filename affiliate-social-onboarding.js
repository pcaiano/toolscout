...
export async function ensureAffiliateSocialOnboardingSchema(env){
  ...
  await env.DB.exec(`CREATE TRIGGER IF NOT EXISTS trg_affiliate_social_on_insert
    ...
    BEGIN
      ...
      ON CONFLICT(tool_slug) DO UPDATE SET
        status='pending',source='d1_trigger',affiliate_url=excluded.affiliate_url,
        queued_at=datetime('now'),processed_at=NULL,updated_at=datetime('now');
    END;`);
  await env.DB.exec(`CREATE TRIGGER IF NOT EXISTS trg_affiliate_social_on_update
