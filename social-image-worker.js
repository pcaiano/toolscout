...
async function generate(env, prompt) {
  // Quality first: use the 9B model in production. The 4B fallback is used only
  // for transient model errors, not to mask an exhausted shared daily quota.
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024">
    <defs>
      <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
        ...
        <stop offset="1" stop-color="#18324D"/>
      </linearGradient>
    </defs>
    <rect width="1024" height="1024" fill="url(#bg)"/>
    <circle cx="512" cy="420" r="176" fill="#F5F7FB" fill-opacity="0.08"/>
    <circle cx="512" cy="420" r="84" fill="none" stroke="#2AAEFF" stroke-width="10"/>
  </svg>`;
}
...
async function brandSmoke(env) {
  return composeBrandStream(
    env,
    new Blob([brandSmokeBaseSvg()], { type: 'image/svg+xml' }).stream(),
    'discovery'
  );
}
...
export default {
  async fetch(request, env) {
    ...
    // Quota-free end-to-end test of Cloudflare Images, overlays and typography.
    if (url.pathname === '/brand-smoke') {
      try {
        const branded = await brandSmoke(env);
        return new Response(branded.body, {
          status: 200,
          headers: responseHeaders('discovery', 'brand-smoke-no-ai')
        });
      } catch (error) {
        return json({
    ...
    if (request.method === 'HEAD') {
      return new Response(null, { status: 200, headers: responseHeaders(variant) });
    }
    ...
    const response = new Response(branded.body, {
      status: 200,
      headers: responseHeaders(variant, generated.model)
    });
