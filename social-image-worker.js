...
function responseHeaders(variant, model = 'flux2') {
  return {
    ...
    'x-toolscout-renderer': 'flux2-brand-composer-v10',
    'x-toolscout-image-variant': variant,
    'x-toolscout-image-model': model,
    'x-toolscout-brand-composer': 'svg-overlay',
    'x-toolscout-text-policy': 'deterministic-svg-text-only'
  };
}
...
function brandOverlaySvg(variant) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024">
    ...
    <text x="66" y="970" fill="#D0D5DD" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="600" letter-spacing="1.2">FIND THE RIGHT TOOL. FASTER.</text>
    <g transform="translate(936 66)" stroke="#F5F7FB" stroke-width="3" fill="none" opacity="0.92">
      <circle cx="0" cy="0" r="22"/><circle cx="0" cy="0" r="5" fill="#2AAEFF" stroke="none"/>
...
async function composeBrandStream(env, baseStream, variant) {
  ...
  return (await env.IMAGES.input(baseStream)
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') return json({ ok: true, service: 'toolscout-social-image', renderer: 'flux2-brand-composer-v10', primary: '@cf/black-forest-labs/flux-2-klein-9b', fallback: '@cf/black-forest-labs/flux-2-klein-4b', textPolicy: 'deterministic-svg-text-only', brandComposer: 'svg-overlay', cache: 'edge-cache-enabled' });
