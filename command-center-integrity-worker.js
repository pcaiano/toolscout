...
async function canonicalSnapshot(env,upstream){
  ...
  if(!oauthConnected&&['stale','unavailable','unknown'].includes(ga4Freshness.status))issues.push({metric:'ga4',severity:'warning',reason:ga4Freshness.status});
  const authorityAttempts24=Number(authorityThroughput?.value?.attempts24??authorityThroughput?.attempts24??0);
  for(const row of unrecoveredFailures.ok?unrecoveredFailures.value||[]){
    if(row.engine==='distribution'&&row.mission==='authority_execution_recovery'&&authorityAttempts24>=6)continue;
    issues.push({metric:`engine:${row.engine}:${row.mission}`,severity:'error',reason:row.detail||'unrecovered_failed_run'});
  }
