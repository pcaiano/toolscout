...
async function canonicalSnapshot(env,upstream){
  ...
  const [sessions,trend,outbound,byTool,todayVisitors,last24Visitors,audienceLatest,contentLatest,runsResult]=await Promise.all([
    ...
    all(env,`SELECT visitor_id,path,source,referrer_host,created_at FROM confirmed_visitor_events WHERE created_at>=? ORDER BY created_at ASC`,[todayStart]),
    all(env,`SELECT visitor_id,path,source,referrer_host,created_at FROM confirmed_visitor_events WHERE created_at>=? ORDER BY created_at ASC`,[last24Start]),
  ...
  const points=[];for(let i=29;i>=0;i--){const d=new Date(now.getTime()-i*86400000),key=dayKey(d);points.push({day:key,sessions:trend.ok?(trendMap.get(key)?.size||0):null})}
  ...
  return {
    ...
    sessions:sessions.ok?{status:'observed',last24:finiteOrNull(sessionRow.last24),today:finiteOrNull(sessionRow.today),monthToDate:mtd,dailyAverageMTD:dailyAverage,projectedMonth,lastAllowedAt:sessionRow.lastAllowedAt||null}: {status:'unavailable',last24:null,today:null,monthToDate:null,dailyAverageMTD:null,projectedMonth:null,reason:sessions.error},
    trafficTrend:{status:trend.ok?'observed':'unavailable',metric:'Browser Guard allowed sessions',windowDays:30,points,generatedAt:now.toISOString(),reason:trend.ok?null:trend.error},
...
function mergeAudit(data,audit){
  ...
  d.trafficTruth={...(d.trafficTruth||{}),status:audit.sessions.status==='observed'?'observed':'degraded',primaryMetric:'Browser Guard allowed sessions plus first-party verified outbound navigation',d1:{...(d.trafficTruth?.d1||{}),status:audit.sessions.status,metric:'Browser Guard allowed sessions',last24:audit.sessions.last24,today:audit.sessions.today,monthToDate:audit.sessions.monthToDate,dailyAverageMTD:audit.sessions.dailyAverageMTD,projectedMonth:audit.sessions.projectedMonth,humanOutbound:audit.outbound.humanOutbound,monetizedOutbound:audit.outbound.monetizedOutbound,unmonetizedOutbound:audit.outbound.unmonetizedOutbound,weightedCoverage:audit.outbound.weightedCoverage,commercialTruth:'first_party_verified_navigation'}};
