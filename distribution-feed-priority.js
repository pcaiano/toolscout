...
export async function prioritizedDistributionFeed(request,env,format='json'){
  ...
  const prioritySet=new Set(priority),urls=[...new Set([...priority,...rest])].slice(0,100);
  const items=urls.map(url=>({id:url,url,title:label(url),date_modified:new Date().toISOString(),toolscout_priority:prioritySet.has(url)?'gsc-observed':'catalog'}));
  if(format==='xml'){
