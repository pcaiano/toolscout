...
async function buildBrief(env,family,{issue=false,task=null}={}){
  ...
  if(comparison){
    ...
    t={linkedin:`${base}?utm_source=linkedin&${common}`,x:`${base}?utm_source=x&${common}`,bluesky:`${base}?utm_source=bluesky&${common}`,pinterest:`${base}?utm_source=pinterest&${common}`,threads:`${base}?utm_source=threads&${common}`};
  }
  ...
  const tagEditorialSocial=(value,channel)=>tagOwned((()=>{try{const u=new URL(String(value||'https://trytoolscout.org/'));u.searchParams.set('utm_source',channel);u.searchParams.set('utm_medium','organic_social');u.searchParams.set('utm_campaign',humanAcquisitionSprintActive()?'human_acquisition_sprint':'content_engine_v21');u.searchParams.set('utm_content',family);return u.toString()}catch{return 'https://trytoolscout.org/'}})(),channel);
  t={linkedin:tagOwned(t.linkedin,'linkedin'),x:tagOwned(t.x,'x'),bluesky:tagOwned(t.bluesky,'bluesky'),pinterest:tagEditorialSocial(t.pinterest,'pinterest'),threads:tagEditorialSocial(t.threads,'threads')};
  const prompt=[
    ...
    `Threads target: ${t.threads}. Threads is initially editorial-only; do not use direct affiliate URLs until platform-specific policy onboarding says otherwise.`
  ].filter(Boolean).join('\n');
  if(issue){
