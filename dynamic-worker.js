...
async function trackedClick(request,env){
  try{
    const b=await request.json();const tool=String(b.tool||'').slice(0,100),intent=String(b.intent||'general').slice(0,100),session=String(b.session||'').slice(0,100);let source=String(b.source||'recommendation').slice(0,100);if(!tool||!session)return Response.json({error:'invalid_event'},{status:400,headers:cors()});if(isOwnerRequest(request))source='internal-test';await env.DB.prepare("INSERT INTO click_events (tool_slug,intent_slug,session_id,source,created_at) VALUES (?,?,?,?,datetime('now'))").bind(tool,intent,session,source).run();return Response.json({ok:true},{headers:cors()});
  }catch{return Response.json({error:'invalid_request'},{status:400,headers:cors()});}
}
async function trackedSearch(request,env){
  try{
    const b=await request.json();const intent=String(b.intent||'general').slice(0,100),session=String(b.session||'').slice(0,100),profile=JSON.stringify(b.profile||{}).slice(0,2000);let source=String(b.source||'search').slice(0,100);if(!session)return Response.json({error:'invalid_event'},{status:400,headers:cors()});if(isOwnerRequest(request))source='internal-test';await env.DB.prepare("INSERT INTO search_events (intent_slug,profile_json,session_id,source,created_at) VALUES (?,?,?,?,datetime('now'))").bind(intent,profile,session,source).run();return Response.json({ok:true},{headers:cors()});
  }catch{return Response.json({error:'invalid_event'},{status:400,headers:cors()});}
}
