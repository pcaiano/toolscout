...
export async function runGrowthSupervisorAudit(env){
  ...
  const gscAge=ageHours(gscTruthGeneratedAt),organicAge=ageHours(organic?.generatedAt);
  ...
  const gscIndexed=n(gscReality?.indexHealth?.indexed),gscInspected=n(gscReality?.indexHealth?.inspected),gscIndexIssues=n(gscReality?.indexHealth?.notIndexed),gscCanonicalMismatches=n(gscReality?.indexHealth?.canonicalMismatches);
  ...
  const ctx={
    ...
    seo_geo_aio:{...humans.seo_geo_aio,e24:0,e7:seoInterventions,lastExecutionAgeHours:organicAge,activeOpportunities:n(byType.search),gscAgeHours:gscAge,organicActionsAgeHours:organicAge,gscImpressions,gscClicks,gscIndexed,gscInspected,gscIndexIssues,gscCanonicalMismatches,verifiedBacklinks,verifiedReferringDomains,backlinkAcquisitionRequired,backlinkAttempts24,backlinkAttempts7,backlinkLastVerifiedAgeHours,backlinkThroughputGap,backlinkStagnating,authorityQueue},
