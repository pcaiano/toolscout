...
function backlinkPolicy(c){
  const verifiedBacklinks=n(c.verifiedBacklinks),observedBacklinks=Math.max(verifiedBacklinks,n(c.observedBacklinks)),verifiedReferringDomains=n(c.verifiedReferringDomains);
  ...
  return{
    ...
    backlink_slowdown_allowed:false,
    ...
    backlink_observed:observedBacklinks,
    backlink_domain_authority:n(c.domainAuthority),
    backlink_domain_authority_source:c.domainAuthoritySource||null,
...
export async function runGrowthSupervisorAudit(env){
  ...
  const seRankingBacklinks=seRankingFresh?n(seRankingBacklinkTruth?.metrics?.backlinks):0;
  const seRankingDofollowBacklinks=seRankingFresh?n(seRankingBacklinkTruth?.metrics?.dofollowBacklinks):0;
  const seRankingDofollowReferringDomains=seRankingFresh?n(seRankingBacklinkTruth?.metrics?.dofollowReferringDomains):0;
  const seRankingDomainAuthority=seRankingFresh?n(seRankingBacklinkTruth?.metrics?.domainAuthority??seRankingBacklinkTruth?.metrics?.domainInlinkRank):0;
  const observedBacklinks=Math.max(verifiedBacklinks,seRankingBacklinks);
  ...
  const backlinkAcquisitionRequired=true;
  ...
  const ctx={
    distribution:{...humans.distribution,e24:d24.count+s24.count,e7:d7.count+s7.count,lastExecutionAgeHours:(Math.max(d7.last,s7.last)?(now-Math.max(d7.last,s7.last))/HOUR:Infinity),activeOpportunities:n(byType.surface)+n(byType.tool),verifiedBacklinks,observedBacklinks,domainAuthority:seRankingDomainAuthority,domainAuthoritySource:seRankingFresh?'SE Ranking':null,verifiedReferringDomains,backlinkAcquisitionRequired,backlinkAttempts24,backlinkAttempts7,backlinkLastVerifiedAgeHours,backlinkThroughputGap,backlinkStagnating,authorityQueue},
    ...
    seo_geo_aio:{...humans.seo_geo_aio,e24:0,e7:seoInterventions,lastExecutionAgeHours:organicAge,activeOpportunities:n(byType.search),gscAgeHours:gscAge,organicActionsAgeHours:organicAge,gscImpressions,gscClicks,gscIndexed,gscInspected,gscIndexIssues,gscCanonicalMismatches,gscRedirected,gscDiscoveredNotIndexed,gscUnknownToGoogle,verifiedBacklinks,observedBacklinks,domainAuthority:seRankingDomainAuthority,domainAuthoritySource:seRankingFresh?'SE Ranking':null,verifiedReferringDomains,backlinkAcquisitionRequired,backlinkAttempts24,backlinkAttempts7,backlinkLastVerifiedAgeHours,backlinkThroughputGap,backlinkStagnating,authorityQueue,googleOrganicHumans24h:strictLearning.googleOrganic24,googleOrganicHumans7d:strictLearning.googleOrganic7,googleOrganicNewsHumans24h:strictLearning.googleOrganicNews24,googleOrganicNewsHumans7d:strictLearning.googleOrganicNews7,searchHumanPaths},
