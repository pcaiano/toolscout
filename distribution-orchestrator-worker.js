...
async function coordinateGrowthOpportunities(env){
  ...
  const SEO_HYGIENE_BASELINE_AT='2026-09-21T20:47:35.000Z';
  const postHygieneSearchReality=Number.isFinite(Date.parse(String(gscReality?.generatedAt||'')))&&Date.parse(String(gscReality?.generatedAt||''))>=Date.parse(SEO_HYGIENE_BASELINE_AT);
  ...
  const normalizedGscPages=directGscPages
    ...
    .slice(0,50);
  const authorityConcentration=normalizedGscPages
  ...
  for(const row of normalizedGscPages){
    ...
    const signals={
      ...
      authority_concentration_target:concentrationTarget,
      authority_concentration_rank:concentrationRank,
      authority_concentration_limit:20,
      north_star:HUMAN_ACQUISITION_SPRINT.northStar,
      audience_strategy:audienceStrategy.phase,
      acquisition_mode:'existing_demand_search'
    };
    const meaningful=row.impressions>=20||row.clicks>0;
    const concentrationBoost=concentrationTarget?15:0;
    const basePriority=Math.min(100,(audienceStrategy.borrowedFirst?row.priority+(meaningful?10:2):row.priority)+seoBoost+concentrationBoost);
  ...
  if(humanSprintActive()){
    for(const target of HUMAN_ACQUISITION_GSC_TARGETS){
      ...
      const signals={
        ...
        evidence_settled_through:gscSignals?.endDate||null,
  ...
  return {ok:true,active,surfaces:surfaceCount,tools:toolCount,affiliate:affiliateCount,catalog:catalogCount,news:newsCount,search:searchCount,searchEvidenceGeneratedAt:organicGrowth?.generatedAt||null,gscSnapshot:{generatedAt:gscReality?.generatedAt||gscSignals?.generatedAt||null,startDate:gscSignals?.startDate||null,endDate:gscSignals?.endDate||null,pages:directGscPages.length,directOpportunities:normalizedGscPages.length,indexTechnicalOpportunities:searchRealityTechnical.length,indexInspected:Number(gscReality?.indexHealth?.inspected||0),indexPass:Number(gscReality?.indexHealth?.indexed||0),postHygieneEvidence:postHygieneSearchReality,seoHygieneBaselineAt:SEO_HYGIENE_BASELINE_AT,authorityConcentration:authorityConcentration.map(row=>({path:row.pathname,impressions:row.impressions,position:row.position,priority:row.priority}))},catalogEvidenceGeneratedAt:catalogFreshness?.generatedAt||null,inputFreshness,audienceStrategy,humanAcquisitionSprint:{active:humanSprintActive(),...HUMAN_ACQUISITION_SPRINT}};
}

async function executeCatalogImpactReviewTask(env,task={}){
  if(task?.action!=='catalog_impact_review'||task?.subject_type!=='news_update')return{ok:false,verified:false,reason:'unsupported_catalog_impact_task'};
  const row=await env.DB.prepare(`SELECT signal_json,priority_score FROM growth_opportunity_state WHERE opportunity_key=? LIMIT 1`).bind(String(task.opportunity_key||'')).first().catch(()=>null);
  let signal={};try{signal=JSON.parse(row?.signal_json||'{}')}catch{}
  const slug=String(signal?.tool_slug||task?.subject_key||'').toLowerCase().replace(/[^a-z0-9-]/g,'');
  const staticTools=await growthAssetJson(env,'/data/tools.json',[]);
  const staticTool=(Array.isArray(staticTools)?staticTools:[]).find(x=>String(x?.slug||'').toLowerCase()===slug)||null;
  const runtimeCandidate=slug?await env.DB.prepare(`SELECT profile_json,status,verified_at,updated_at FROM catalog_runtime_candidates WHERE tool_slug=? AND status IN ('published','admitted_coverage','quality_hold') LIMIT 1`).bind(slug).first().catch(()=>null):null;
  const runtimeState=slug?await env.DB.prepare(`SELECT quality_status,content_changed,last_checked_at,last_change_at FROM catalog_runtime_state WHERE tool_slug=? LIMIT 1`).bind(slug).first().catch(()=>null):null;
  let runtimeProfile=null;try{runtimeProfile=JSON.parse(runtimeCandidate?.profile_json||'null')}catch{}
  const catalogPresent=Boolean(staticTool||runtimeProfile);
  const sourceUrl=String(signal?.source_url||'').slice(0,2000)||null;
  const title=String(signal?.title||'').slice(0,800)||null;
  const summary=String(signal?.summary||'').slice(0,1800)||null;
  const catalogRelevant=Boolean(signal?.verified_source)||/(pricing|price|plan|feature|integration|release|launch|api|automation|security|capabilit|availability)/i.test(`${title||''} ${summary||''}`);
  const outcome=!catalogPresent?'reviewed_no_catalog_profile':catalogRelevant?'reviewed_first_party_refresh_due':'reviewed_no_material_catalog_change';
  const evidence={proof_kind:'catalog_impact_review_completed',opportunity_key:task.opportunity_key||null,tool_slug:slug||null,catalog_present:catalogPresent,catalog_relevant:catalogRelevant,outcome,source_url:sourceUrl,title,quality_status:runtimeState?.quality_status||null,last_catalog_check_at:runtimeState?.last_checked_at||null};
  await env.DB.prepare(`INSERT INTO catalog_runtime_events(event_id,tool_slug,event_type,status,detail,evidence_json,created_at)
    VALUES(?,?,?,?,?,?,datetime('now'))`).bind('cat_'+crypto.randomUUID(),slug||null,'catalog_impact_reviewed','completed',catalogPresent?'Software update reviewed against the current ToolScout catalog profile.':'Software update reviewed; no matching ToolScout catalog profile requires mutation.',JSON.stringify(evidence).slice(0,8000)).run().catch(()=>{});
...
async function runGrowthExecutionContractCycle(env){
  ...
  const runCatalogTask=async(task)=>{
    if(task?.source_kind==='opportunity'&&task?.subject_type==='catalog_gap')return{task:await executeCatalogGrowthTask(env,task)};
    if(task?.source_kind==='opportunity'&&task?.subject_type==='news_update'&&task?.action==='catalog_impact_review')return{task:await executeCatalogImpactReviewTask(env,task)};
    const verify=await contractVerifyCatalogBatch(env);
    const admit=await contractAdmitCatalogCandidates(env);
    return{verify,admit};
  };
  if(selectedInternalLane==='catalog_cycle')await runInternal('catalog_cycle',runCatalogTask);
  if(selectedInternalLane!=='catalog_cycle'){
    const catalogImpactReady=await env.DB.prepare("SELECT 1 ok FROM growth_execution_contract WHERE executor='catalog_cycle' AND action='catalog_impact_review' AND status IN ('pending','stalled','deferred') LIMIT 1").first().catch(()=>null);
...
async function runGrowthRndAudit(env){
  ...
  const seoCfg=supervisors.get('seo_geo_aio')?.cfg||{};
  const external24=Math.max(0,Number(supervisors.get('growth_brain')?.external_executions_24h||0));
  const acquisitionMin=Math.max(1,Number(growthCfg.external_execution_min_24h||15));
  const backlinkCfg={...seoCfg,...distCfg};
  ...
  for(const item of playCandidates.slice(0,acquisitionSlots)){
    ...
    await env.DB.prepare(`INSERT INTO growth_rnd_experiments(experiment_key,experiment_type,subject_key,hypothesis,action_json,status,risk_class,expected_signal,created_at,last_evaluated_at,updated_at)
      VALUES(?,?,?,?,?,'active','bounded',?,datetime('now'),datetime('now'),datetime('now'))
      ON CONFLICT(experiment_key) DO UPDATE SET experiment_type=excluded.experiment_type,subject_key=excluded.subject_key,hypothesis=excluded.hypothesis,action_json=excluded.action_json,status='active',expected_signal=excluded.expected_signal,last_evaluated_at=datetime('now'),updated_at=datetime('now')
      WHERE growth_rnd_experiments.experiment_type IS NOT excluded.experiment_type
         OR growth_rnd_experiments.subject_key IS NOT excluded.subject_key
         OR growth_rnd_experiments.hypothesis IS NOT excluded.hypothesis
         OR growth_rnd_experiments.action_json IS NOT excluded.action_json
...
async function learnEconomics(env){
  ...
  for(const row of q.results||[]){
    ...
    const learningStmt=env.DB.prepare(`INSERT INTO distribution_economic_learning(surface_slug,baseline_score,learned_score,human_sessions_30d,outbound_clicks_30d,monetized_outbound_30d,confirmed_revenue_30d,currency,economic_boost,last_observed_at,created_at,updated_at,browser_confirmed_sessions_30d,session_to_outbound_rate,monetization_rate,evidence_grade,paid_policy_decision,observed_cost,observed_cost_currency,observed_roi)
      ...
      .bind(row.surface_slug,baseline,learned,sessions,outbound,monetized,revenue,metric?.currency||null,boost,sessions,sessionToOutbound,monetizationRate,grade,policy.decision,cost?.cost_amount??null,cost?.currency??null,policy.roi);
    const opportunityStmt=env.DB.prepare(`UPDATE distribution_opportunities
      ...
      .bind(learned,sessions,outbound,opportunityRevenue,boost,row.surface_slug,learned,sessions,outbound,opportunityRevenue,boost);
    plans.push({learningStmt,opportunityStmt});
  }
  ...
  try{
    for(let i=0;i<plans.length;i+=batchSize){
      ...
      const results=await env.DB.batch(chunk.flatMap(x=>[x.learningStmt,x.opportunityStmt]));
      for(let j=0;j<chunk.length;j++){
        const a=results[j*2],b=results[j*2+1];
        const learningChanges=Number(a?.meta?.changes||a?.changes||0);
        const opportunityChanges=Number(b?.meta?.changes||b?.changes||0);
        if(learningChanges||opportunityChanges)updated++;
      }
    }
  }catch(error){
    return{ok:false,observed:metrics.length,evaluated,updated,reason:'economic_learning_batch_write_failed',detail:String(error?.message||error).slice(0,500),evidence_basis:'strict_verified_human_sessions'};
  }
  ...
  return{ok:true,observed:metrics.length,evaluated,updated,positive_boosts:positive,surfaces_with_evidence:withEvidence,paid_experiments_measuring:paidMeasuring,evidence_basis:'strict_verified_human_sessions',write_policy:'material_change_only',write_mode:'d1_batch_v1',batch_size:batchSize};
}
...
if(u.pathname==='/api/growth/opportunities/public-reconcile'&&request.method==='POST'){
  if(!(await growthEscalationHandoffOk(request)))return Response.json({error:'unauthorized'},{status:401,headers:H});
  return Response.json(await runWithLedger(env,{engine:'growth',mission:'opportunity_coordination',triggerName:'make_handoff_search_reality',singleFlightMinutes:15},()=>coordinateGrowthOpportunities(env)),{headers:H});
}
