...
async function prepareEditorial(env,url,type){const targets=[['community','Hacker News / Show HN','https://news.ycombinator.com/'],['community','Indie Hackers','https://www.indiehackers.com/'],['community','Relevant Reddit communities','https://www.reddit.com/']];let n=0;for(const [channel,name,target] of targets){const id=`ed_${crypto.randomUUID()}`;const a=angle(url,type);const title=type==='comparison'?`We compared ${new URL(url).pathname.split('/').pop()?.replace(/\.html$/,'').replace(/-/g,' ')}`:`ToolScout research: ${new URL(url).pathname.split('/').pop()?.replace(/\.html$/,'').replace(/-/g,' ')}`;const body=`${a}\n\nToolScout: ${url}\n\nPrepared automatically. Review community rules and context before posting.`;await env.DB.prepare(`INSERT INTO distribution_editorial_queue(queue_id,asset_url,channel_type,target_name,target_url,angle,suggested_title,suggested_body,status,human_required,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?, 'prepared',1,datetime('now'),datetime('now'))`).bind(id,url,channel,name,target,a,safe(title,300),safe(body,3000)).run();n++;}return n;}
async function fanout(env,url){const type=classify(url);await env.DB.prepare(`INSERT INTO distribution_asset_state(asset_url,asset_type,first_seen_at,last_seen_at,distributed_at) VALUES(?,?,datetime('now'),datetime('now'),datetime('now')) ON CONFLICT(asset_url) DO UPDATE SET asset_type=excluded.asset_type,last_seen_at=datetime('now'),distributed_at=COALESCE(distribution_asset_state.distributed_at,datetime('now'))`).bind(url,type).run();const editorial=await prepareEditorial(env,url,type);await env.DB.prepare(`INSERT INTO distribution_events(event_id,event_type,status,asset_type,asset_id,destination_url,detail,observed_at,created_at) VALUES(?,?,?,?,?,?,?,datetime('now'),datetime('now'))`).bind(`asset_${crypto.randomUUID()}`,'asset_distribution_triggered','completed',type,url,url,`Event-driven fanout prepared: syndication feed exposure, Submission Engine eligibility, Vendor Amplification eligibility and ${editorial} editorial/community drafts.`).run();return {ok:true,asset_url:url,asset_type:type,editorialPrepared:editorial};}
async function scanNew(request,env){let r;try{r=await env.ASSETS.fetch(new Request(new URL('/sitemap.xml',request.url)));}catch{return{ok:false,scanned:0,newAssets:0}}if(!r.ok)return{ok:false,scanned:0,newAssets:0};const xml=await r.text();const urls=[...xml.matchAll(/<loc>([^<]+)<\/loc>/g)].map(m=>m[1]).filter(u=>/^https:\/\/trytoolscout\.org\//.test(u)&&/(best-|\-vs-|alternatives|compare)/i.test(u));let added=0;for(const url of urls.slice(0,150)){const row=await env.DB.prepare('SELECT asset_url FROM distribution_asset_state WHERE asset_url=?').bind(url).first();if(row)continue;await fanout(env,url);added++;}return{ok:true,scanned:urls.length,newAssets:added};}
export default {async fetch(request,env,ctx){const u=new URL(request.url);if(u.pathname==='/api/distribution/orchestrate'&&request.method==='POST'){if(!(await auth(request,env)))return Response.json({error:'unauthorized'},{status:401,headers:H});let b={};try{b=await request.json()}catch{return Response.json({error:'invalid_json'},{status:400,headers:H})}if(!b.asset_url||!/^https:\/\/trytoolscout\.org\//.test(String(b.asset_url)))return Response.json({error:'valid_toolscout_asset_url_required'},{status:400,headers:H});return Response.json(await fanout(env,String(b.asset_url)),{headers:H});}if(u.pathname==='/api/distribution/orchestrate/scan'&&request.method==='POST'){if(!(await auth(request,env)))return Response.json({error:'unauthorized'},{status:401,headers:H});return Response.json(await scanNew(request,env),{headers:H});}if(u.pathname==='/api/distribution/economic-learning'&&request.method==='POST'){if(!(await auth(request,env)))return Response.json({error:'unauthorized'},{status:401,headers:H});return Response.json(await learnEconomics(env),{headers:H});}if(u.pathname==='/api/distribution/editorial-queue'&&request.method==='GET'){if(!(await auth(request,env)))return Response.json({error:'unauthorized'},{status:401,headers:H});const q=await env.DB.prepare(`SELECT queue_id,asset_url,channel_type,target_name,target_url,angle,suggested_title,suggested_body,status,human_required,updated_at FROM distribution_editorial_queue ORDER BY created_at DESC LIMIT 100`).all();return Response.json({status:'connected',items:q.results||[]},{headers:H});}return base.fetch(request,env,ctx);},async scheduled(event,env,ctx){await learnEconomics(env).catch(()=>({ok:false}));if(base.scheduled)await base.scheduled(event,env,ctx);if(event?.cron==='15 3 * * *')await scanNew(new Request('https://trytoolscout.org/'),env);}};
